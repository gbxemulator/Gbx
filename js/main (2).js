...
/* ============================================================
   11. SAUVEGARDES — SUPABASE (fichier .srm)
   ============================================================
   Système à deux niveaux :
   • Slot 0 (AUTO) : watcher toutes les 3s, sync si le .srm change
   • Slots 1-3 (MANUELS) : boutons SAUVEGARDER / CHARGER

   Modifier le CHECK dans la table SQL :
     slot int NOT NULL CHECK (slot BETWEEN 0 AND 3)
   ============================================================ */

// ── Lire le .srm depuis le FS Emscripten en base64 ───────────
function _getSrmBase64() {
    try {
        ...
        return btoa(binary);
    } catch(e) { console.warn('[GBX] getSrmBase64:', e); return null; }
}

// ── Écrire le .srm dans le FS et recharger ───────────────────
...
// ── Upsert Supabase (commun auto + manuel) ───────────────────
async function _upsertSlot(gameId, slot, base64) {
    const { error } = await _supa
        .from('save_states')
        .upsert({
            user_id:  GBX.user.id,
    ...
    if (error) { console.error('[GBX] upsertSlot:', error); return false; }
    return true;
}

// ── AUTO-SAVE : watcher sur le .srm toutes les 3s ────────────
...
function startAutoSave() {
    ...
    _autoSaveTimer = setInterval(async () => {
        ...
        try {
            ...
            const ok = await _upsertSlot(GBX.currentGame.id, 0, b64);
            if (ok) {
                console.info('[GBX] 💾 Auto-save syncé');
                const btn = document.getElementById('btn-save-state');
                if (btn) { btn.classList.remove('auto-saved'); void btn.offsetWidth; btn.classList.add('auto-saved'); }
            }
        } catch(e) { /* silencieux */ }
    }, 3_000);
}
...
// ── Sauvegarder dans un slot manuel (1-3) ────────────────────
async function _saveToSlot(gameId, slot) {
    const base64 = _getSrmBase64();
    if (!base64) { showToast('⚠️ Lance un jeu d\'abord !', 'error'); return false; }
    const ok = await _upsertSlot(gameId, slot, base64);
    if (!ok) { showToast('❌ Erreur lors de la sauvegarde.', 'error'); return false; }
    return true;
}
...
async function _loadFromSlot(gameId, slot) {
    const { data, error } = await _supa
        ...
        .eq('slot', slot)
        .single();
    if (error || !data) { showToast('❌ Aucune sauvegarde dans ce slot.', 'error'); return false; }
    try {
        const gm = GBX.emulator?.gameManager || window.EJS_emulator?.gameManager;
        if (!gm) { showToast('⚠️ Lance un jeu d\'abord !', 'error'); return false; }
        _applySrmBase64(data.data);
        return true;
    } catch(e) {
...
function _buildSlotsGrid(containerId, filledSlots, onSlotClick, includeAuto = false) {
    const grid = document.getElementById(containerId);
    if (!grid) return;
    grid.innerHTML = '';
    const startSlot = includeAuto ? 0 : 1;
    for (let slot = startSlot; slot <= 3; slot++) {
        ...
        label.textContent = slot === 0 ? '🔄 Auto-save' : 'Slot ' + slot;
...
// ── Ouvrir la modale SAUVEGARDER (slots 1-3 uniquement) ──────
async function openSaveStateModal() {
    ...
    _buildSlotsGrid('save-slots-grid', slots, async (slot) => {
        ...
        if (ok) { closeModal(); showToast('✅ Sauvegardé dans le slot ' + slot + ' !', 'success'); }
    }, false);
    openModal('save-state');
}

// ── Ouvrir la modale CHARGER (auto + slots 1-3) ───────────────
async function openLoadStateModal() {
    ...
    _buildSlotsGrid('load-slots-grid', slots, async (slot, filled) => {
        ...
        if (ok) { closeModal(); showToast('📂 ' + (slot === 0 ? 'Auto-save' : 'Slot ' + slot) + ' chargé !', 'success'); }
    }, true);
...
function setupSaveStateUI() {
    const btnLoad = document.getElementById('btn-load-state');
    const btnSave = document.getElementById('btn-save-state');
    btnLoad?.addEventListener('click', () => { addPressEffect(btnLoad); openLoadStateModal(); });
    btnSave?.addEventListener('click', () => { addPressEffect(btnSave); openSaveStateModal(); });

...
async function launchGame(game) {
    ...
    window.EJS_ready = () => {
        ...
        // Empêcher le canvas EJS de voler le focus sur mobile (cause scintillement)
        setTimeout(() => {
            const ejsCanvas = document.querySelector('#ejs-container canvas');
            if (ejsCanvas) {
                ejsCanvas.setAttribute('tabindex', '-1');
                ejsCanvas.addEventListener('focus', (e) => {
                    e.preventDefault();
                    ejsCanvas.blur();
                }, true);
            }

...
async function launchGame(game) {
    ...
    window.EJS_ready = () => {
        ...
        if (GBX.user) {
            _restoreIngameSave(game.id);
        }
    };

    // Colorisation GBC automatique pour les jeux Game Boy Classic
    // Pour GB : on re-restaure la save après la colorisation (qui peut réinitialiser le core)
    // Note: gambatte_gb_colorization retiré car il réinitialise le core
    // et écrase la save ingame restaurée sur les jeux GB
...
async function _restoreIngameSave(gameId) {
    // Fetch la save Supabase immédiatement
    let saveArr = null;
    try {
    ...
    if (!saveArr) { setTimeout(() => startAutoSave(), 10_000); return; }

    // Étape 1 : écrire dans IndexedDB AVANT le lancement
    // EmulatorJS utilise IDBFS — syncfs() copie IndexedDB → FS WASM au boot
    // En écrivant ici on s'assure que le core lit la bonne save dès le démarrage
    try {
        const idbPath = _getIDBSavePath(gameId);
        if (idbPath) {
            await _writeToIDB(idbPath, saveArr);
            // Aussi créer l'entrée dossier si elle n'existe pas
    ...
    // Étape 2 : aussi injecter dans le FS WASM dès qu'il est dispo
    // (pour GBC/GBA qui peuvent recharger en cours de partie via loadSaveFiles)
    let attempts = 0;
    const MAX = 60;

    const tryInject = () => {
        attempts++;
        if (attempts > MAX) { setTimeout(() => startAutoSave(), 10_000); return; }

        try {
            const gm = window.EJS_emulator?.gameManager;
            if (!gm?.FS) { setTimeout(tryInject, 100); return; }

            const path = gm.functions?.getSaveFilePath?.() || '';
            const dir = path ? path.substring(0, path.lastIndexOf('/')) : null;
            if (!dir) { setTimeout(tryInject, 100); return; }

            try { gm.FS.mkdir(dir); } catch(e) {}
            gm.FS.writeFile(path, saveArr);
            try { gm.functions.loadSaveFiles(); } catch(e) {}
...
// Construire le chemin IndexedDB du .srm à partir du game_id
...
function startAutoSave() {
    ...
    _autoSaveTimer = setInterval(async () => {
        ...
        try {
            ...
            // Ne pas sync si moins de 20% des bytes sont non-FF (save non initialisée)
            const nonFF = Array.from(data).filter(b => b !== 0xFF).length;
            if (nonFF < data.length * 0.20) return;

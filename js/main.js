...
/* ============================================================
   ...
   IMPORTANT — ce système ne touche PAS au CSS existant quand
   il n'est pas actif. Il lit les positions depuis getBoundingClientRect
   au moment où l'édition démarre, et les restitue inline.
   ============================================================ */

// ── Groupes éditables ─────────────────────────────────────────
// id        : id HTML de l'élément
// minScale  : facteur de réduction max
// maxScale  : facteur d'agrandissement max
const GBX_LAYOUT_ELEMENTS = [
    ...
    { id: 'controls-center',  minScale: 0.6, maxScale: 1.6 },
...
const _layoutEdit = {
    ...
    snapshot:  null,     // positions avant édition (pour Annuler)
    current:   null,     // positions en cours d'édition
};
...
// ── Lire la position d'un élément depuis le DOM ───────────────
// Retourne { x (vw%), y (vh%), w (px), h (px), scale }
// On stocke la position du coin top-left en % viewport,
// le scale est lu depuis le style inline s'il existe.
function _readElementPos(el) {
...
// ── Appliquer une position inline sur un élément ──────────────
// On passe UNIQUEMENT en position:fixed + left/top en vw/vh.
// On préserve le transform original s'il n'y a pas de scale custom.
function _applyPos(el, pos) {
    ...
    el.style.right          = '';
    el.style.bottom         = '';
    // On replace translateY(-50%) par rien — la position est déjà calculée
    el.style.transform      = `scale(${pos.scale})`;
    el.style.transformOrigin = 'top left';
}

// ── Prendre un snapshot complet des positions DOM actuelles ───
function _snapshotFromDOM() {
    const snap = {};
    GBX_LAYOUT_ELEMENTS.forEach(({ id }) => {
        const el = document.getElementById(id);
        if (el) snap[id] = _readElementPos(el);
    });
    return snap;
}

// ── Charger le layout Supabase ────────────────────────────────
...
// ── Sauvegarder le layout Supabase ───────────────────────────
async function _saveLayout(modeData) {
    if (!GBX.user) return;
    // Charger d'abord pour ne pas écraser l'autre orientation
...
// ── Appliquer un layout sauvegardé aux éléments ───────────────
// Appelé au chargement / changement d'orientation.
// N'est appelé QUE si un layout existe en base — ne touche jamais
// au CSS par défaut si l'utilisateur n'a rien sauvegardé.
function _applyLayoutToDOM(modeData) {
    if (!modeData) return;
    GBX_LAYOUT_ELEMENTS.forEach(({ id }) => {
        const el  = document.getElementById(id);
...
async function startLayoutEdit() {
    ...
    // 1. Lire les positions actuelles depuis le DOM
    //    (qu'elles viennent du CSS par défaut ou d'un layout sauvegardé)
    const domSnap = _snapshotFromDOM();

    // 2. Snapshot pour Annuler — on mémorise aussi les styles inline
    //    existants pour les restaurer si l'utilisateur annule
    _layoutEdit.snapshot = {
        positions: JSON.parse(JSON.stringify(domSnap)),
        inlineStyles: {},
    };
    GBX_LAYOUT_ELEMENTS.forEach(({ id }) => {
        const el = document.getElementById(id);
    ...
    // 3. Figer les positions en styles inline fixed
    //    à partir du snapshot DOM (les boutons restent là où ils sont)
    _layoutEdit.current = domSnap;
    GBX_LAYOUT_ELEMENTS.forEach(({ id }) => {
        const el  = document.getElementById(id);
        const pos = domSnap[id];
        if (el && pos) _applyPos(el, pos);
    });

    // 4. Activer l'overlay + surbrillance
    document.getElementById('layout-edit-overlay')?.classList.add('active');
    GBX_LAYOUT_ELEMENTS.forEach(({ id, minScale, maxScale }) => {
        const el = document.getElementById(id);
        if (!el) return;
...
function _attachDragPinch(el, id, minScale, maxScale) {
    ...
    // On capture touchstart en non-passive pour pouvoir stopPropagation
    // et empêcher le système pressButton de se déclencher
    function onTouchStart(e) {
        e.stopPropagation();
        // Pas de preventDefault — on veut juste stopper la propagation
    }
    ...
    // Au touchstart on initialise startRect et startScale
...
async function _applyLayoutOnLoad() {
    ...
    if (saved[mode]) _applyLayoutToDOM(saved[mode]);
}
...
function setupLayoutEditor() {
    ...
    // Bouton Annuler — restaure les styles d'avant
    ...
    // Bouton Défaut — efface le layout sauvegardé et remet le CSS par défaut

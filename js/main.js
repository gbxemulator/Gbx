...
async function startLayoutEdit() {
    ...
    // 1. Mémoriser les styles inline existants pour Annuler.
    //    On ne touche PAS aux positions ici — le CSS continue de gérer
    //    l'affichage exactement comme avant l'activation du mode édition.
    ...
    // 2. Activer l'overlay + surbrillance (les boutons restent à leur place)
...
function _attachDragPinch(el, id, minScale, maxScale) {
    ...
    let startScale    = 1;
    let startDist     = 0;
    let fixedAtStart  = false; // l'élément a-t-il été figé en fixed ?

    // Au touchstart : figer l'élément en position:fixed à sa position DOM réelle,
    // puis initialiser les valeurs de référence pour le drag/pinch.
    function onTouchStartFull(e) {
        e.stopPropagation();

        // Figer en fixed à partir de la position réelle (getBoundingClientRect)
        // seulement si pas encore fait (le CSS peut utiliser right/bottom/transform)
        if (!fixedAtStart) {
        ...
        const m2      = el.style.transform?.match(/scale\(([\d.]+)\)/);
        startScale    = m2 ? parseFloat(m2[1]) : 1;
    ...
    function onTouchEnd(e) {
        e.stopPropagation();
        // Re-initialiser pour un éventuel nouveau drag sur le même élément
        activeTouches = Array.from(e.touches);

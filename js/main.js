...
/* ============================================================
   ...
   - portrait  : dpad, ab-buttons, ss-group (SELECT+START groupés)
   - landscape : dpad, ab-buttons, btn-l, btn-r, btn-select, btn-start
                 (SELECT et START indépendants, L et R déplaçables)
...
const GBX_LAYOUT_ELEMENTS = {
    portrait: [
        { id: 'dpad',            minScale: 0.6, maxScale: 1.6 },
        { id: 'ab-buttons',      minScale: 0.6, maxScale: 1.6 },
        { id: 'controls-center', minScale: 0.6, maxScale: 1.6 }, // SELECT+START groupés
    ],
    landscape: [
        { id: 'dpad',            minScale: 0.6, maxScale: 1.6 },
        { id: 'ab-buttons',      minScale: 0.6, maxScale: 1.6 },
        { id: 'btn-l',           minScale: 0.6, maxScale: 1.6 },
        { id: 'btn-r',           minScale: 0.6, maxScale: 1.6 },
        { id: 'controls-center', minScale: 0.6, maxScale: 1.6 }, // SELECT seul (btn-start est séparé)
        { id: 'btn-start',       minScale: 0.6, maxScale: 1.6 },
    ],
};
...
async function startLayoutEdit() {
    ...
    _layoutEdit.snapshot = { inlineStyles: {} };
    _layoutEdit.current  = {};
    ...
    _layoutElements().forEach(({ id, minScale, maxScale }) => {
        const el = document.getElementById(id);
        if (!el) return;
        el.classList.add('layout-editable');
        _attachDragPinch(el, id, minScale, maxScale);
    });
}
...
function _attachDragPinch(el, id, minScale, maxScale) {
    ...
    function onTouchStartFull(e) {
        ...
        if (!fixedAtStart) {
            ...
            // Lire le scale existant (inline ou CSS)
            const computedTransform = window.getComputedStyle(el).transform;
            const inlineMatch = el.style.transform?.match(/scale\(([\d.]+)\)/);
            let sc = 1;
            if (inlineMatch) {
                sc = parseFloat(inlineMatch[1]);
            } else if (computedTransform && computedTransform !== 'none') {
                // Extraire le scale de la matrix CSS si possible
                const matMatch = computedTransform.match(/matrix\(([\d.]+)/);
                if (matMatch) sc = parseFloat(matMatch[1]);
            }
            el.style.position        = 'fixed';
            ...
            el.style.transformOrigin = 'top left';
            _layoutEdit.current[id]  = { x: r.left / vw * 100, y: r.top / vh * 100, scale: sc };
            fixedAtStart = true;
        }
    ...
    function onTouchMove(e) {
        e.stopPropagation();
        if (!fixedAtStart || activeTouches.length === 0) return;

        const vw = window.innerWidth;
        const vh = window.innerHeight;

        if (e.touches.length === 1) {
            const dx  = e.touches[0].clientX - activeTouches[0].clientX;
            const dy  = e.touches[0].clientY - activeTouches[0].clientY;
            ...
            _layoutEdit.current[id] = { ..._layoutEdit.current[id], x: newX, y: newY };
            ...
            el.style.transformOrigin = 'top left';
            _layoutEdit.current[id] = { ..._layoutEdit.current[id], scale: newScale };
        }
    }
    ...
    el.addEventListener('touchstart',  onTouchStartFull, { passive: true, capture: true });
    ...
    _dragListeners.set(el, { onTouchStartFull, onTouchMove, onTouchEnd });
}

function _detachDragPinch(el) {
    const L = _dragListeners.get(el);
    if (!L) return;
    el.removeEventListener('touchstart',  L.onTouchStartFull, { capture: true });

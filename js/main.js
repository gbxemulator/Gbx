...
async function launchGame(game) {
    ...
    // ── Pré-charger la save depuis Supabase pour l'injecter via EJS_onLoadSave ──
    ...
    if (GBX.user) {
        try {
            const { data } = await _supa
                ...
                .maybeSingle();
            if (data?.save_data) {
                // Stocker les bytes en mémoire — ils seront injectés via EJS_onLoadSave
                const base64 = data.save_data.split(',')[1] ?? data.save_data;
    ...
    // ── Hook EJS natif : appelé par EJS quand il veut charger la SRAM ──────────
    // C'est le seul moment garanti où on peut injecter des bytes dans le core.
    // EJS attend un Uint8Array en retour (ou undefined pour ignorer).
    window.EJS_onLoadSave = () => {
        if (GBX._pendingSaveBytes) {
            console.info('[GBX] EJS_onLoadSave → injection', GBX._pendingSaveBytes.length, 'bytes');
            return GBX._pendingSaveBytes;
        }
        return undefined;
    };

    // ── Hook EJS natif : appelé par EJS quand il veut sauvegarder la SRAM ─────
    // Reçoit un Uint8Array des données de save. On l'envoie à Supabase.
    ...
    window.EJS_ready = () => {
        ...
        // Autosave toutes les 10s via readSaveFile (fallback si EJS_onSaveSave ne se déclenche pas)
        if (GBX.user) {
    ...
    // EJS_onGameStart — appelé quand le core est lancé et prêt
...
function getSavePath(game) {
    ...
    // Gambatte utilise .sav, mGBA utilise .srm
    const ext      = core === 'gambatte' ? '.sav' : '.srm';
    return `/data/saves/${coreDir}/${baseName}${ext}`;
}
...
async function loadSaveForGame(game) {
    ...
    if (data?.save_data) {
        // Supporte "data:...;base64,XXXX" et base64 brut
        ...
        try {
            ...
            console.warn('[GBX] loadSaveForGame error:', e);
        }
    }
}
...
async function uploadSaveToServer(game, saveData, silent = false) {
    ...
    return new Promise((resolve) => {
        ...
        const reader = new FileReader();
        reader.onload = async () => {
            // reader.result = "data:application/octet-stream;base64,XXXX"
            // On stocke le data URL complet pour pouvoir le relire facilement

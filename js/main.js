...
function showEjectBubble() {
    ...
    if (eject) {
        ...
        setTimeout(() => { eject.classList.add('shake'); }, 0);
...
function toggleSpeed() {
    if (!GBX.isRunning || !GBX.emulator || !GBX.speedReady) {
        ...
        if (btn) {
            ...
            setTimeout(() => { btn.classList.add('shake'); }, 0);
...
function setupGameControls() {
    ...
    Object.entries(buttonMap).forEach(([btnId, ejsRealIndex]) => {
        ...
        // Sur mobile, touchend génère des events synthétiques mouse + hover
        // qui causent un flash CSS au relâchement. On les bloque tous.
        ...
        el.addEventListener('touchend', (e) => {
            ...
            // Bloquer TOUS les events synthétiques post-touch pendant 500ms
            // (mouse + click + hover) — c'est eux qui causent le flash au relâchement
            _isTouching = true;
            setTimeout(() => { _isTouching = false; }, 500);
        }, { passive: false });
        ...
        // Bloquer mousedown/mouseup/click/mouseover synthétiques post-touch
        el.addEventListener('mousedown',  (e) => { if (_isTouching) { e.preventDefault(); return; } pressButton(el, ejsRealIndex, true);  });
        el.addEventListener('mouseup',    (e) => { if (_isTouching) { e.preventDefault(); return; } pressButton(el, ejsRealIndex, false); });
        el.addEventListener('mouseleave', ()  => { if (_isTouching) return; pressButton(el, ejsRealIndex, false); });
        el.addEventListener('mouseover',  (e) => { if (_isTouching) e.preventDefault(); });
        el.addEventListener('click',      (e) => { if (_isTouching) e.preventDefault(); });
    });
...
function _openSaveConfirm() {
    if (!GBX.user) {
        const btn = document.getElementById('btn-save');
        if (btn) { btn.classList.remove('shake'); setTimeout(() => { btn.classList.add('shake'); btn.addEventListener('animationend', () => btn.classList.remove('shake'), { once: true }); }, 0); }
...
function setupPcControls() {
    ...
    volBtn?.addEventListener('click', (e) => {
        ...
        if (!GBX.isRunning) {
            ...
            volBtn.classList.remove('shake'); setTimeout(() => { volBtn.classList.add('shake'); }, 0);
    ...
    screenBtn?.addEventListener('click', (e) => {
        ...
        if (!GBX.isRunning) {
            ...
            screenBtn.classList.remove('shake'); setTimeout(() => { screenBtn.classList.add('shake'); }, 0);

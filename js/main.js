...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    updateLRButtons('');       // L/R actifs par défaut (pas de jeu chargé)
...
function updateLRButtons(type) {
    ...
    // Désactiver uniquement pour gb/gbc — actif pour gba ET sans jeu chargé
    const shouldDisable = (type === 'gb' || type === 'gbc');
    lBtn.disabled = shouldDisable;
    rBtn.disabled = shouldDisable;
    lBtn.classList.toggle('disabled', shouldDisable);
    rBtn.classList.toggle('disabled', shouldDisable);
}
...
async function startLayoutEdit() {
    ...
    // Retirer disabled sur btn-l et btn-r dans les deux modes
    // (disabled quand jeu gb/gbc chargé — pointer-events:none bloquerait les touches)
    ['btn-l', 'btn-r'].forEach(btnId => {
        const el = document.getElementById(btnId);
        if (!el) return;
        _layoutEdit.snapshot.inlineStyles[btnId] = _layoutEdit.snapshot.inlineStyles[btnId] || {};
        _layoutEdit.snapshot.inlineStyles[btnId]._wasDisabled      = el.disabled;
        _layoutEdit.snapshot.inlineStyles[btnId]._hadDisabledClass = el.classList.contains('disabled');
        el.disabled = false;
...
function _stopLayoutEdit() {
    ...
    ['btn-l', 'btn-r'].forEach(id => {
        ...
        if (!el || !s) return;
        if (s._wasDisabled)      el.disabled = true;
        if (s._hadDisabledClass) el.classList.add('disabled');
    });
...
function _attachDrag(el, id) {
    ...
    function onTouchStart(e) {
        ...
        // Si l'élément est dans un sous-conteneur positionné (ex: controls-center),
        // on le téléporte dans #gameboy pour que ses coordonnées abs soient correctes.
        if (!fixedAtStart) {
            // Lire la position AVANT tout changement DOM
            const r  = el.getBoundingClientRect();
            const gb = document.getElementById('gameboy');
            // Mémoriser le parent original pour restauration
            ...
            // Téléporter dans #gameboy si pas déjà enfant direct
            if (el.parentElement !== gb) gb.appendChild(el);
            // Utiliser r.left/r.top lus AVANT le déplacement DOM
            // #gameboy est à 0,0 dans le viewport donc ces coords sont directement utilisables
            el.style.position        = 'absolute';
            ...
            el.style.transformOrigin = '';
            el.style.zIndex          = '10';
            _layoutEdit.current[id]  = { x: r.left, y: r.top };
            fixedAtStart = true;
        }
    ...
    function onTouchMove(e) {
        ...
        const gb  = document.getElementById('gameboy');
        const gbR = gb ? gb.getBoundingClientRect() : { width: window.innerWidth, height: window.innerHeight };
        ...
        // Limiter aux dimensions de #gameboy (containing block pour position:absolute)

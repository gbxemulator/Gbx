...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    setupCommunautePage();    // Page communauté avec bulles
    ...
    fetchUserCount();          // Compteur total d'utilisateurs inscrits

    // Forcer will-change via JS — le CSS seul ne suffit pas sur Chromium mobile
    document.querySelectorAll('.dpad-btn, .ab-btn, .lr-btn, .ss-btn, .top-btn, #btn-eject, #controls-main, #controls-top').forEach(el => {
...
function updateLRButtons(type) {
    ...
    // Désactiver uniquement sur GB et GBC (pas de boutons L/R sur ces consoles)
    const disable = (type === 'gb' || type === 'gbc');
    lBtn.disabled = disable;
    rBtn.disabled = disable;
    lBtn.classList.toggle('disabled', disable);
    rBtn.classList.toggle('disabled', disable);
}
...
function setupGameControls() {
    ...
    // Slide-to-press : suivre le doigt en touchmove pour appuyer sur
    // le bouton sous le doigt, même si le touchstart était sur un autre.
    // On garde une trace du bouton actuellement "pressé" par chaque touch.
    const activeTouchButtons = new Map(); // touchIdentifier → { el, ejsIndex }

    function getButtonAtPoint(x, y) {
        // Masquer temporairement le pointer-events pour que elementFromPoint
        // traverse vers l'élément en dessous si besoin
        const hit = document.elementFromPoint(x, y);
        if (!hit) return null;
        // Chercher le bouton dans les ancêtres (ex: texte dans un <button>)
        let node = hit;
        while (node) {
            if (node.id && buttonMap[node.id] !== undefined) return node;
    ...
    Object.entries(buttonMap).forEach(([btnId, ejsRealIndex]) => {
        ...
        el.addEventListener('touchstart', (e) => {
            Array.from(e.changedTouches).forEach(t => {
                activeTouchButtons.set(t.identifier, { el, ejsRealIndex });
                pressButton(el, ejsRealIndex, true);
            });
        }, { passive: true });

        el.addEventListener('touchend', (e) => {
            Array.from(e.changedTouches).forEach(t => {
                const active = activeTouchButtons.get(t.identifier);
                if (active) {
                    pressButton(active.el, active.ejsRealIndex, false);
                    activeTouchButtons.delete(t.identifier);
                }
            });
        }, { passive: true });
        ...
        el.addEventListener('touchmove', (e) => {
            Array.from(e.changedTouches).forEach(t => {
                const newBtn = getButtonAtPoint(t.clientX, t.clientY);
                const active = activeTouchButtons.get(t.identifier);

                if (active && (!newBtn || newBtn !== active.el)) {
                    // Le doigt a quitté le bouton actif
                    pressButton(active.el, active.ejsRealIndex, false);
                    activeTouchButtons.delete(t.identifier);
                }

                if (newBtn && (!active || newBtn !== active.el)) {
                    const newIndex = buttonMap[newBtn.id];
                    if (newIndex !== undefined) {
                        // Le doigt est entré sur un nouveau bouton
                        activeTouchButtons.set(t.identifier, { el: newBtn, ejsRealIndex: newIndex });
                        pressButton(newBtn, newIndex, true);
                    }
                }
            });
        }, { passive: true });
...
function setupKeyboard() {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        // Touche G quand modal-cheat ouverte → fermer (priorité avant isModalOpen)
        ...
        // Touche Q (A AZERTY = bouton B) quand modal save-confirm ouverte → fermer
...
function setupAuthUI() {
    ...
    document.getElementById('form-forgot-pwd')?.addEventListener('submit', handleForgotPwd);

    document.getElementById('link-forgot-pwd')?.addEventListener('click', (e) => {
        e.preventDefault();
        openModal('forgot-pwd');
    });
...
function openProfileModal() {
    ...
    // Pré-remplir discord
    const discordInput = document.getElementById('profile-discord-input');
    if (discordInput) discordInput.value = GBX.user.discord || '';

    // Pré-remplir email
    const emailInput = document.getElementById('profile-email-input');
    if (emailInput) emailInput.value = GBX.user.email || '';

    // Toggle statut
    const statusToggle = document.getElementById('profile-show-status');
    if (statusToggle) {
        statusToggle.checked = GBX.user.show_status !== false;
        _updateStatusLabel();
        statusToggle.onchange = async () => {
            GBX.user.show_status = statusToggle.checked;
            _updateStatusLabel();
            await _supa.from('users').update({ show_status: statusToggle.checked }).eq('id', GBX.user.id);
        };
    }

    // Bouton save discord
    document.getElementById('btn-save-discord')?.addEventListener('click', async () => {
        const val = document.getElementById('profile-discord-input').value.trim();
        GBX.user.discord = val || null;
        await _supa.from('users').update({ discord: val || null }).eq('id', GBX.user.id);
        showToast('✅ Discord mis à jour !', 'success');
    }, { once: false });

    // Bouton save email (doit passer par Supabase Auth)
    document.getElementById('btn-save-email')?.addEventListener('click', async () => {
        ...
        if (!newEmail) return;
        const { error } = await _supa.auth.updateUser({ email: newEmail });
        if (error) { showToast('❌ ' + error.message, 'error'); return; }
        await _supa.from('users').update({ email: newEmail }).eq('id', GBX.user.id);
        GBX.user.email = newEmail;
        showToast('✅ Email mis à jour ! Vérifie ta boîte mail.', 'success');
    }, { once: false });

    // Bouton save mdp
    document.getElementById('btn-save-pwd')?.addEventListener('click', async () => {
        const newPwd     = document.getElementById('profile-pwd-input').value;
        const confirmPwd = document.getElementById('profile-pwd-confirm-input').value;
        if (!newPwd) return;
        if (newPwd.length < 6) { showToast('❌ Minimum 6 caractères.', 'error'); return; }
        if (newPwd !== confirmPwd) { showToast('❌ Les mots de passe ne correspondent pas.', 'error'); return; }
        const { error } = await _supa.auth.updateUser({ password: newPwd });
        if (error) { showToast('❌ ' + error.message, 'error'); return; }
        document.getElementById('profile-pwd-input').value         = '';
        document.getElementById('profile-pwd-confirm-input').value = '';
    ...
    // Dernière connexion
    const lastSeenEl = document.getElementById('modal-profile-lastseen');
    if (lastSeenEl) {
        if (GBX.user.show_status === false) {
            lastSeenEl.textContent = '🕐 Dernière connexion : Inconnue';
        } else if (GBX.user.last_seen) {
            lastSeenEl.textContent = '🕐 ' + _formatLastSeen(GBX.user.last_seen);
        } else {
...
function _updateStatusLabel() {
    const toggle = document.getElementById('profile-show-status');
...
function _renderFavGame() {
    ...
    if (GBX.user.fav_game) {
        // Chercher d'abord dans GBX_POKEMON_GAMES (nouveau format avec platform)
        const pokeEntry = (GBX_POKEMON_GAMES || []).find(e =>
            (typeof e === 'string' ? e : e.name) === GBX.user.fav_game
        );
        let badge = '';
        if (pokeEntry && typeof pokeEntry === 'object' && pokeEntry.platform) {
            const label = GBX_PLATFORMS?.[pokeEntry.platform] || pokeEntry.platform.toUpperCase();
            badge = `<span class="cheat-game-type-badge" style="font-size:9px;padding:1px 5px;vertical-align:middle;margin-right:6px;">${label}</span>`;
        } else {
            // Fallback : chercher dans GBX_GAMES (jeux émulés)
            const game = GBX_GAMES?.find(g => g.name === GBX.user.fav_game);
            if (game) badge = _getConsoleBadge(game.type);
        }
...
function _buildFavGameSelect() {
    ...
    (GBX_POKEMON_GAMES || []).forEach(entry => {
        const name     = typeof entry === 'string' ? entry : entry.name;
        const platform = typeof entry === 'string' ? null  : entry.platform;
        const opt      = document.createElement('option');
        opt.value      = name;
        const label    = platform && GBX_PLATFORMS?.[platform]
            ? '[' + GBX_PLATFORMS[platform] + '] ' + name
            : name;
        opt.textContent = label;
        if (name === GBX.user.fav_game) opt.selected = true;
        sel.appendChild(opt);
    });
...
function closeModal() {
    if (typeof _cancelKeyCapture === "function") _cancelKeyCapture();
...
async function handleLogin(e) {
    ...
    // Mettre à jour last_seen si l'utilisateur a activé le statut
    if (GBX.user.show_status !== false) {
        await _supa.from('users')
            .update({ last_seen: new Date().toISOString() })
            .eq('id', GBX.user.id);
        GBX.user.last_seen = new Date().toISOString();
    }
...
function updateAuthUI() {
    ...
    const rowTouches       = document.getElementById('btn-row-touches');
    const rowMobileLogged  = document.getElementById('btn-row-mobile-logged');
    const rowCommMobile    = document.getElementById('btn-row-communaute-mobile-logged');
    ...
    if (GBX.user) {
        ...
        if (rowTouches)          rowTouches.style.display          = 'flex';
        if (rowMobileLogged)     rowMobileLogged.style.display     = 'flex';
        if (rowCommMobile)       rowCommMobile.style.display       = 'flex';
        const btnContactUnlogged = document.getElementById('btn-contact-unlogged');
        if (btnContactUnlogged) btnContactUnlogged.style.display = 'none';
    } else {
        ...
        if (rowTouches)        rowTouches.style.display        = 'none';
        if (rowMobileLogged)   rowMobileLogged.style.display   = 'none';
        if (rowCommMobile)     rowCommMobile.style.display     = 'none';
        const btnContactUnlogged = document.getElementById('btn-contact-unlogged');
        if (btnContactUnlogged) btnContactUnlogged.style.display = '';
...
function setupContactForm() {
    ...
    btn?.addEventListener('click', () => {
        ...
        if (GBX.contactOpen) {
            ...
            if (isMobile) {
                setTimeout(() => {
                    document.getElementById('sidebar')?.scrollTo({ top: 99999, behavior: 'smooth' });
                }, 50);
            }
        }
    });
    ...
    document.getElementById('form-contact')?.addEventListener('submit', (e) => {
        ...
        window.location.href = `mailto:GbxEmulator@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(message)}`;

        showToast('✉️ Client mail ouvert !', 'success');
        e.target.reset();
...
function _buildCheatCodeList(game) {
    ...
    const addBtn = document.getElementById('cheat-add-btn');
    addBtn.textContent = 'Ajouter';
    addBtn._editIdx = null;
    addBtn.onclick = () => {
        ...
        if (addBtn._editIdx !== null && addBtn._editIdx !== undefined) {
            // Mode édition — mettre à jour le code existant
            const existing = GBX_CHEAT_CUSTOM[game.id][addBtn._editIdx];
            const wasActive = GBX_CHEAT_ACTIVE[game.id]?.has('custom_' + addBtn._editIdx);
            if (wasActive) {
                _deactivateCheat(game, 'custom_' + addBtn._editIdx, existing);
            }
            GBX_CHEAT_CUSTOM[game.id][addBtn._editIdx] = { name: nameVal, code: codeVal };
            addBtn._editIdx = null;
            addBtn.textContent = 'Ajouter';
...
function _buildCheatItem(cheat, key, game, isCustom) {
    ...
    // Défilement si le titre dépasse 24 caractères
    const SCROLL_THRESHOLD = 24;
    if (cheat.name.length > SCROLL_THRESHOLD) {
        name.classList.add('scrolling');
        // On double le texte pour un scroll en boucle fluide
        const span = document.createElement('span');
        span.textContent = cheat.name + '\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0' + cheat.name;
    ...
    if (cheat.note) {
        ...
        note.className   = 'cheat-code-note';
        note.textContent = '⚠ ' + cheat.note;
    ...
    // Boutons édition + suppression (custom uniquement)
    if (isCustom) {
        const customIdx = parseInt(key.replace('custom_', ''));

        // Bouton crayon — remet le code dans le formulaire pour édition
        const editBtn = document.createElement('button');
        editBtn.className   = 'cheat-code-edit';
        editBtn.textContent = '✏️';
        editBtn.title       = 'Modifier ce code';
        editBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            const nameInput = document.getElementById('cheat-custom-name');
            const codeInput = document.getElementById('cheat-custom-code');
            const addBtn    = document.getElementById('cheat-add-btn');
            if (!nameInput || !codeInput || !addBtn) return;
            // Pré-remplir le formulaire
            nameInput.value = cheat.name;
            codeInput.value = cheat.code;
            // Changer le bouton Ajouter en Mettre à jour
            addBtn.textContent = '💾 Mettre à jour';
            addBtn._editIdx = customIdx;
...
function _applyCheatToEmulator(cheat, enable) {
    try {
        ...
        if (!Module?._set_cheat || !Module?._reset_cheat || !Module?.stringToNewUTF8 || !Module?._free) return;

        if (enable) {
            // Activation directe : index = nombre de codes déjà actifs pour CE jeu
            const gameId = GBX.currentGame?.id;
            const activeCount = gameId ? (GBX_CHEAT_ACTIVE[gameId]?.size || 0) - 1 : 0;
            const ptr = Module.stringToNewUTF8(cheat.code);
            Module._set_cheat(Math.max(0, activeCount), 1, ptr);
            Module._free(ptr);
        } else {
            // Désactivation : reset total puis réapplication de tous les codes encore actifs
            Module._reset_cheat();
            let idx = 0;
            const gameId = GBX.currentGame?.id;
            if (!gameId) return;
            ...
            active.forEach(key => {
                let c = null;
                if (key.startsWith('preset_')) c = presets[parseInt(key.replace('preset_', ''))];
                else if (key.startsWith('custom_')) c = customs[parseInt(key.replace('custom_', ''))];
                if (!c) return;
                const ptr = Module.stringToNewUTF8(c.code);
                Module._set_cheat(idx++, 1, ptr);
                Module._free(ptr);
            });
        }
    } catch(e) {
...
function _reapplyActiveCheatForGame(gameId) {
    ...
    active.forEach(key => {
        let cheat = null;
        if (key.startsWith('preset_')) cheat = presets[parseInt(key.replace('preset_', ''))];
        else if (key.startsWith('custom_')) cheat = customs[parseInt(key.replace('custom_', ''))];
        if (!cheat) return;
        const ptr = Module.stringToNewUTF8(cheat.code);
        Module._set_cheat(idx++, 1, ptr);
        Module._free(ptr);
...
function setupExtraButtons() {
    ...
    // Bouton Touches/Boutons → modal keymap (PC)
    ...
    // Bouton Touches mobile (connecté)
    document.getElementById('btn-touches-mobile')?.addEventListener('click', () => {
        toggleSidebar(false);
        buildKeyMapGrid();
        openModal('keymap');
    });

    // Bouton Communauté mobile connecté
    document.getElementById('btn-communaute-mobile')?.addEventListener('click', () => {
        toggleSidebar(false);
        document.getElementById('communaute-page')?.classList.add('open');
        _loadCommunauteUsers();
    });

    // Bouton Contact mobile connecté
    document.getElementById('btn-contact-mobile')?.addEventListener('click', () => {
        GBX.contactOpen = true;
        const wrap = document.getElementById('contact-form-wrap');
        const btn  = document.getElementById('btn-contact');
    ...
    // Bouton Contact PC (dans la rangée connecté)
    document.getElementById('btn-contact-pc')?.addEventListener('click', () => {
        GBX.contactOpen = true;
        const wrap = document.getElementById('contact-form-wrap');
        const btn  = document.getElementById('btn-contact');
        if (wrap) wrap.classList.add('open');
    ...
    // Bouton Contact non-connecté PC
    document.getElementById('btn-contact-unlogged')?.addEventListener('click', () => {
        GBX.contactOpen = true;
        const wrap = document.getElementById('contact-form-wrap');
        const btn  = document.getElementById('btn-contact');
        if (wrap) wrap.classList.add('open');
...
const GBX_KEYMAP_ACTIONS = [
    ...
    { id: 'dpad-up',    label: '↑ Haut',    ejsIndex: 4  },
    { id: 'dpad-down',  label: '↓ Bas',     ejsIndex: 5  },
    { id: 'dpad-left',  label: '← Gauche',  ejsIndex: 6  },
    { id: 'dpad-right', label: '→ Droite',  ejsIndex: 7  },
...
function applyKeyMap(map) {
    // Vider complètement AZERTY_MAP et reconstruire depuis le mapping
    Object.keys(AZERTY_MAP).forEach(k => delete AZERTY_MAP[k]);

    // Actions jeu (boutons + directions)
    const ejsMap = {
        'btn-a': 8, 'btn-b': 0, 'btn-select': 2, 'btn-start': 3,
        'btn-l': 10, 'btn-r': 11,
        'dpad-up': 4, 'dpad-down': 5, 'dpad-left': 6, 'dpad-right': 7,
    };

    Object.entries(ejsMap).forEach(([id, ejsIndex]) => {
        const code = map[id] || GBX_KEYMAP_DEFAULT[id];
        if (code) AZERTY_MAP[code] = { ejsIndex, btnId: id };
    });

    GBX.customKeys = map;
}

// Capture en cours : { btn, cancelFn } ou null
// Un seul mode attente à la fois (clavier ET manette partagent ce slot)
let _activeCapture = null;

function _cancelAnyCapture() {
    if (_activeCapture) {
        _activeCapture.cancelFn();
        _activeCapture = null;
    }
}
// Alias pour closeModal (appelé plus tôt dans le fichier)
function _cancelKeyCapture() { _cancelAnyCapture(); }
...
function buildKeyMapGrid() {
    ...
    grid.querySelectorAll('.keymap-key-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // Déjà en attente → RESET la touche
            if (btn.classList.contains('waiting')) {
                _cancelAnyCapture();
                const actionId   = btn.dataset.action;
                const currentMap = loadKeyMap();
                delete currentMap[actionId];
                btn.dataset.current = GBX_KEYMAP_DEFAULT[actionId] || '';
                btn.textContent     = '—';
            ...
            // Annuler toute capture en cours (clavier ou manette)
            _cancelAnyCapture();
            ...
            const cancelFn = () => {
                ...
                if (_activeCapture?.btn === btn) _activeCapture = null;
            };

            const onKey = (e) => {
                e.preventDefault();
                e.stopImmediatePropagation();
                if (e.code === 'Escape') { cancelFn(); return; }

                const actionId   = btn.dataset.action;
                const currentMap = loadKeyMap();
                const conflict   = Object.entries(currentMap).find(([k, v]) => v === e.code && k !== actionId);
                if (conflict) {
                    const lbl = GBX_KEYMAP_ACTIONS.find(a => a.id === conflict[0])?.label || conflict[0];
                    showToast('⚠️ Touche déjà utilisée pour "' + lbl + '"', 'error');
                    cancelFn();
                    return;
                }
            ...
            _activeCapture = { btn, cancelFn };
            document.addEventListener('keydown', onKey, true);
        });
    });
}
/* ── Gamepad mapping ─────────────────────────────────────── */
const GBX_GAMEPAD_KEY     = 'gbx-gamepad-map';
const GBX_GAMEPAD_DEFAULT = {}; // vide par défaut — aucun bouton mappé

// Labels standards boutons gamepad (layout STANDARD)
const GAMEPAD_BTN_LABELS = [
    'A','B','X','Y',
    'LB','RB','LT','RT',
    'Back','Start',
    'LS','RS',
...
function loadGamepadMap() {
    try {
        const s = localStorage.getItem(GBX_GAMEPAD_KEY);
        if (s) return JSON.parse(s);
    } catch(_) {}
    return { ...GBX_GAMEPAD_DEFAULT };
}

function saveGamepadMap(map) {
    try { localStorage.setItem(GBX_GAMEPAD_KEY, JSON.stringify(map)); } catch(_) {}
}

function gamepadBtnLabel(btnIndex) {
    return (btnIndex !== null && btnIndex !== undefined)
        ? (GAMEPAD_BTN_LABELS[btnIndex] || 'Btn' + btnIndex)
        : '—';
}

// Polling gamepad en jeu
// État précédent des boutons pour détecter press/release
const _gpPrevState = new Map(); // padIndex → {buttons: [], axes: []}
...
function startGamepadPolling() {
    ...
    const poll = () => {
        const map  = loadGamepadMap(); // relit à chaque frame pour voir les changements
        ...
        for (let pi = 0; pi < pads.length; pi++) {
            ...
            const prev = _gpPrevState.get(pi) || { buttons: [], axes: [0,0,0,0] };
            const gm   = GBX.emulator?.gameManager;
            ...
            pad.buttons.forEach((btn, i) => {
                ...
                if (wasPressed === isPressed) return; // pas de changement

                // Trouver l'action mappée sur ce bouton
                for (const [actionId, btnIndex] of Object.entries(map)) {
                    if (btnIndex !== i) continue;
                    const action = GBX_KEYMAP_ACTIONS.find(a => a.id === actionId);
                    if (!action) continue;

                    if (action.ejsIndex >= 0) {
                        // Bouton B manette → ferme la modal save-confirm si ouverte
                        if (actionId === 'btn-b' && isPressed) {
                            const saveModal = document.getElementById('modal-save-confirm');
                            if (saveModal?.classList.contains('open')) {
                                closeModal();
                                continue;
                        ...
                        // Action jeu normale → simulateInput
                        const el = document.getElementById(actionId);
                        if (el && !el.disabled) el.classList.toggle('pressed', isPressed);
                        if (gm) try { gm.simulateInput(0, action.ejsIndex, isPressed ? 1 : 0); } catch(_) {}
                    } else if (isPressed) {
                        // Actions spéciales → déclencher seulement au press
                        if (actionId === '_menu') {
                            const btn2 = document.getElementById('btn-menu');
                            if (btn2) addPressEffect(btn2);
                            toggleSidebar(!GBX.sidebarOpen);
                        } else if (actionId === '_fast') {
                            const btn2 = document.getElementById('btn-speed');
                            if (btn2) addPressEffect(btn2);
                            toggleSpeed();
                        } else if (actionId === '_save') {
                            const btn2 = document.getElementById('btn-save');
                            if (btn2 && btn2.style.display !== 'none') {
                                addPressEffect(btn2);
                                _handleSaveKey();
                            }
                        } else if (actionId === '_cheat') {
                            const btn2 = document.getElementById('btn-cheat');
                            if (btn2 && btn2.style.display !== 'none') {
                                addPressEffect(btn2);
                                const cheatModal = document.getElementById('modal-cheat');
                                if (cheatModal?.classList.contains('open')) closeModal();
                                else openCheatModal();
                            }
                        }
                    }
            ...
            // ── Axes joystick gauche → D-PAD (seulement si mappé dans les axes) ──
            // On utilise les axes UNIQUEMENT si le mapping gamepad a au moins une entrée
            // pour ne pas interférer avec le clavier si aucune manette n'est configurée
            const hasAnyMapping = Object.keys(map).length > 0;
            if (hasAnyMapping && gm) {
                const axH     = pad.axes[0];
                const axV     = pad.axes[1];
                const prevAxH = prev.axes[0] || 0;
                const prevAxV = prev.axes[1] || 0;
                const DEAD    = 0.5;

                // Gauche
                if ((axH < -DEAD) !== (prevAxH < -DEAD))
                    try { gm.simulateInput(0, 6, axH < -DEAD ? 1 : 0); } catch(_) {}
                // Droite
                if ((axH > DEAD) !== (prevAxH > DEAD))
                    try { gm.simulateInput(0, 7, axH > DEAD ? 1 : 0); } catch(_) {}
                // Haut
                if ((axV < -DEAD) !== (prevAxV < -DEAD))
                    try { gm.simulateInput(0, 4, axV < -DEAD ? 1 : 0); } catch(_) {}
                // Bas
                if ((axV > DEAD) !== (prevAxV > DEAD))
                    try { gm.simulateInput(0, 5, axV > DEAD ? 1 : 0); } catch(_) {}
            }
...
// Construire la grille manette
function buildGamepadGrid() {
    const grid = document.getElementById('keymap-grid-gamepad');
    if (!grid) return;

    const map    = loadGamepadMap();
    const status = document.getElementById('keymap-gamepad-status');
    const pads   = navigator.getGamepads();
    const activePad = Array.from(pads).find(p => p);
    if (status) {
        status.textContent = activePad ? ('🟢 ' + activePad.id.slice(0, 30)) : 'Aucune manette détectée';
        status.className   = 'keymap-gamepad-status' + (activePad ? ' connected' : '');
    }

    grid.innerHTML = '';
    GBX_KEYMAP_ACTIONS.forEach(action => {
        const currentBtn = map[action.id] ?? null;
        const row = document.createElement('div');
        row.className = 'keymap-row';
        row.innerHTML = `
            <span class="keymap-action-label">${action.label}</span>
            <button class="keymap-key-btn keymap-gamepad-btn" data-action="${action.id}" data-current="${currentBtn}">
                ${gamepadBtnLabel(currentBtn)}
            </button>
        `;
        grid.appendChild(row);
    });

    grid.querySelectorAll('.keymap-gamepad-btn').forEach(btn => {
        btn.addEventListener('click', () => {

            // Déjà en attente → RESET le bouton manette
            if (btn.classList.contains('waiting')) {
                _cancelAnyCapture();
                const actionId   = btn.dataset.action;
                const currentMap = loadGamepadMap();
                delete currentMap[actionId];
                btn.dataset.current = 'null';
                btn.textContent     = '—';
                saveGamepadMap(currentMap);
                startGamepadPolling();
                return;
            }
            ...
            const cancelFn = () => {
                ...
                btn.textContent = gamepadBtnLabel(cur !== 'null' && cur !== '' && cur !== 'undefined' ? parseInt(cur) : null);
                document.removeEventListener('keydown', onEsc);
                if (_activeCapture?.btn === btn) _activeCapture = null;
                startGamepadPolling();
            };

            const onEsc = (e) => { if (e.key === 'Escape') cancelFn(); };
                ...
                for (const pad of pads) {
                    ...
                    for (let i = 0; i < pad.buttons.length; i++) {
                        if (pad.buttons[i].pressed) {
                            ...
                            const actionId   = btn.dataset.action;
                            const currentMap = loadGamepadMap();
                            currentMap[actionId] = i;
                            btn.dataset.current  = i;
                            btn.textContent      = gamepadBtnLabel(i);
                            btn.classList.remove('waiting');
                            _activeCapture = null;
                            saveGamepadMap(currentMap);
                            startGamepadPolling();
                            showToast('✅ Bouton ' + gamepadBtnLabel(i) + ' assigné !', 'success');
                            return;
                        }
                    }
                }
                rafCapture = requestAnimationFrame(pollCapture);
            };
...
function setupKeyMapping() {
    ...
    // Reconnexion / déconnexion manette
    window.addEventListener('gamepadconnected', () => {
        buildGamepadGrid();
        showToast('🎮 Manette connectée !', 'success');
    });
    window.addEventListener('gamepaddisconnected', () => {
        buildGamepadGrid();
        showToast('🎮 Manette déconnectée', 'info');
    });
    ...
    document.getElementById('btn-keymap-reset')?.addEventListener('click', () => {
        ...
        buildKeyMapGrid();
        showToast('↩ Mapping clavier réinitialisé', 'success');
    });

    document.getElementById('btn-keymap-reset-gamepad')?.addEventListener('click', () => {
        saveGamepadMap({});
        buildGamepadGrid();
        showToast('↩ Mapping manette réinitialisé', 'success');
    });
}
...
async function fetchUserCount() {
    try {
        ...
        const plural = document.getElementById('user-plural');
...
async function handleForgotPwd(e) {
    ...
    if (!email) { showToast('Saisis ton adresse email.', 'error'); return; }
    const subject = encodeURIComponent('Mot de passe oublié — ' + email);
    const lines   = "Bonjour,%0A%0AJ'ai oublié mon mot de passe.%0AMon adresse email : " + email + "%0A%0AMerci !";
    window.location.href = 'mailto:GbxEmulator@gmail.com?subject=' + subject + '&body=' + lines;
    closeModal();
    showToast("📧 Ton client mail s'ouvre — envoie le message !", "info");
}

/* ============================================================
   PAGE COMMUNAUTÉ — BULLES
   ============================================================ */

const GBX_BUBBLE_COLORS = [
    "#e63946","#f4a261","#2a9d8f","#457b9d","#06d6a0",
    "#118ab2","#ffd166","#ef476f","#8ecae6","#95d5b2",
    "#f77f00","#c77dff","#4cc9f0","#ff6b6b","#48cae4",
    "#b5838d","#52b788","#ffb703","#e76f51","#a8dadc",
];

let _bubblesData    = [];   // [{user, el, color, left, duration, delay}]
let _poppingLocked  = false; // Une seule bulle éclatée à la fois
let _particlePool   = [];

function _randomBetween(a, b) { return a + Math.random() * (b - a); }
...
function _createBubbleEl(user, index, total, color) {
    ...
    const duration = _randomBetween(8, 14);
    const slotW    = canvas.offsetWidth / total;
    const baseX    = slotW * index + slotW * 0.5 - 34; // centré dans le slot
    const jitter   = _randomBetween(-slotW * 0.15, slotW * 0.15);
    const left     = Math.max(8, Math.min(canvas.offsetWidth - 80, baseX + jitter));
    const delay    = _randomBetween(0, duration * 0.8);

    wrap.style.cssText = `left:${left}px; bottom:-110px; animation-duration:${duration}s; animation-delay:-${delay}s;`;
    ...
    if (hasCustomAvatar) {
        ...
        img.alt       = user.pseudo;
        img.draggable = false;
        img.onerror   = () => { img.src = 'assets/avatar-default.png'; };
    ...
    label.style.background = color + 'cc';
    ...
    // Clic → éclater
    wrap.addEventListener('click', (e) => _popBubble(e, wrap, user, color));

    canvas.appendChild(wrap);
    return wrap;
}

function _popBubble(e, wrap, user, color) {
    if (_poppingLocked) return;
    _poppingLocked = true;

    // Utiliser les coordonnées du clic — fiables même si la bulle est en animation
    const cx = e.clientX;
    const cy = e.clientY;
    ...
    setTimeout(() => {
        wrap.style.display = 'none';
        _showBubbleProfile(user, color, wrap);
    }, 380);
}
...
function _spawnParticles(cx, cy, color) {
    ...
    // Couleurs uniquement basées sur la couleur de la bulle
    const safeColor = (color && color.length === 7 && color[0] === '#') ? color : '#9b59b6';
    ...
    const lighter   = `rgb(${Math.min(255,rgb.r+80)},${Math.min(255,rgb.g+80)},${Math.min(255,rgb.b+80)})`;
    const lighter2  = `rgb(${Math.min(255,rgb.r+40)},${Math.min(255,rgb.g+40)},${Math.min(255,rgb.b+40)})`;
    const darker    = `rgb(${Math.max(0,rgb.r-50)},${Math.max(0,rgb.g-50)},${Math.max(0,rgb.b-50)})`;
    // Toutes les nuances restent dans la teinte de la bulle
    const pieceColors = [safeColor, lighter, lighter2, darker, safeColor, lighter];

    const PIECES = 16;
    const pieces = [];
    for (let i = 0; i < PIECES; i++) {
        const angle = (Math.PI * 2 / PIECES) * i + _randomBetween(-0.25, 0.25);
        const speed = _randomBetween(120, 220);
        pieces.push({
            x: cx, y: cy,
            vx: Math.cos(angle) * speed,
            vy: Math.sin(angle) * speed,
            rot:      _randomBetween(0, Math.PI * 2),
            rotSpeed: _randomBetween(-8, 8),
            w: _randomBetween(10, 24),
            h: _randomBetween(7, 18),
            shape: i % 3,
    ...
    const draw = (now) => {
        ...
        if (t > 1) { cvs.remove(); return; }
        ctx.clearRect(0, 0, cvs.width, cvs.height);
        pieces.forEach(p => {
            const grav = t * t * 160;
            const px   = p.x + p.vx * t;
            const py   = p.y + p.vy * t + grav;
            ...
            if (p.shape === 0) {
                ...
                ctx.bezierCurveTo(-p.w,-p.h, p.w,-p.h, p.w*.5,-p.h*.2);
                ctx.bezierCurveTo(p.w*.3,p.h*.6, -p.w*.3,p.h*.6, -p.w*.5,-p.h*.2);
            } else if (p.shape === 1) {
                ctx.moveTo(0,-p.h); ctx.lineTo(p.w*.7,p.h*.6); ctx.lineTo(-p.w*.6,p.h*.5);
            } else {
                ctx.ellipse(0,0,p.w*.5,p.h*.8,0,0,Math.PI*2);
            }
            ctx.closePath();
            ctx.fill();
            ctx.restore();
...
function _showBubbleProfile(user, color, bubbleWrap) {
    const card    = document.getElementById('bubble-profile-card');
    const overlay = document.getElementById('bubble-overlay');
    if (!card) return;

    // Couleur dynamique
    card.style.setProperty('--bpc-color', color);

    document.getElementById('bpc-avatar').src      = user.avatar || 'assets/avatar-default.png';
    document.getElementById('bpc-pseudo').textContent = user.pseudo;

    // Jeu fav
    const favEl = document.getElementById('bpc-fav-game');
    if (user.fav_game) {
        const game  = GBX_GAMES?.find(g => g.name === user.fav_game);
        const badge = game ? `<span class="cheat-game-type-badge" style="font-size:9px;padding:1px 5px;margin-right:4px">${(game.type||'').toUpperCase()}</span>` : '';
        favEl.innerHTML = '⭐ ' + badge + user.fav_game;
    } else {
        favEl.textContent = '';
    }

    // Dernier jeu
    const lastGameEl = document.getElementById('bpc-last-game');
    lastGameEl.textContent = user.last_game ? '🎮 Dernier jeu : ' + user.last_game : '';

    // Discord
    const discordEl = document.getElementById('bpc-discord');
    discordEl.textContent = user.discord ? '💬 ' + user.discord : '';

    // Dernière connexion
    const seenEl = document.getElementById('bpc-last-seen');
    if (user.show_status === false) {
        seenEl.textContent = '🕐 Dernière connexion : Inconnue';
    } else if (user.last_seen) {
        seenEl.textContent = '🕐 Vu ' + _formatLastSeen(user.last_seen);
    } else {
        seenEl.textContent = '';
    }

    // Afficher overlay + card
    if (overlay) { overlay.style.display = 'block'; overlay.classList.add('visible'); }
    card.style.display = 'block';
    requestAnimationFrame(() => card.classList.add('visible'));

    // Bouton fermer
    document.getElementById('bubble-profile-close').onclick = () => {
        _closeBubbleProfile(card, overlay, bubbleWrap);
    };
    // Clic overlay
    if (overlay) overlay.onclick = () => _closeBubbleProfile(card, overlay, bubbleWrap);
}

function _closeBubbleProfile(card, overlay, bubbleWrap) {
    ...
    setTimeout(() => {
        ...
        // Réapparaître la bulle après 3s — spawn depuis le bas comme une nouvelle bulle
        if (bubbleWrap) {
            setTimeout(() => {
                const canvas = document.getElementById('bubble-canvas');
                if (!canvas) return;
                bubbleWrap.classList.remove('popping');
                // Reset : on replace en bas hors écran, délai 0 = part immédiatement du bas
                bubbleWrap.style.display            = '';
                bubbleWrap.style.animationPlayState = '';
                // Nouveau timing pour un départ naturel depuis le bas
                const newDuration = _randomBetween(8, 14);
                bubbleWrap.style.animationDuration  = newDuration + 's';
                bubbleWrap.style.animationDelay     = '0s';
                // Forcer le reflow pour que l'animation reparte du keyframe 0%
...
async function _loadCommunauteUsers() {
    try {
        const { data: users, error } = await _supa
            .from('users')
            .select('id, pseudo, avatar, fav_game, last_game, discord, last_seen, show_status')
        ...
        const countEl  = document.getElementById('communaute-member-count');
        const pluralEl = document.getElementById('communaute-member-plural');
        if (countEl)  countEl.textContent  = users.length;
        if (pluralEl) pluralEl.textContent = users.length > 1 ? 's' : '';

        // Nettoyer les anciennes bulles
        canvas.querySelectorAll('.bubble-wrap').forEach(b => b.remove());
        _bubblesData = [];

        // Mélanger les couleurs aléatoirement à chaque chargement
        const shuffledColors = [...GBX_BUBBLE_COLORS].sort(() => Math.random() - 0.5);

        users.forEach((user, i) => {
            const color = shuffledColors[i % shuffledColors.length];
            const el    = _createBubbleEl(user, i, users.length, color);
            if (el) _bubblesData.push({ user, el, color });
        });

    } catch(e) { console.warn('[GBX] communauté:', e); }
}

function setupCommunautePage() {
    // Injecter l'overlay dans la page communauté
    const page = document.getElementById('communaute-page');
    if (page && !document.getElementById('bubble-overlay')) {
        const ov = document.createElement('div');
        ov.id = 'bubble-overlay';
        page.appendChild(ov);
    }

    // Boutons Communauté
    ['btn-communaute', 'btn-communaute-pc'].forEach(id => {
    ...
    document.getElementById('communaute-back')?.addEventListener('click', () => {
        document.getElementById('communaute-page')?.classList.remove('open');

...
// à ce qu'EmulatorJS génère en interne (basé sur EJS_gameName sanitisé)
function _getIDBSavePath(gameId) {
    const game = GBX_GAMES.find(g => g.id === gameId);
    if (!game) return null;
    // EmulatorJS sanitise EJS_gameName : retire les caractères spéciaux,
    // remplace les espaces par des underscores, lowercase
    ...
    return `/data/saves/${coreDir}/${sanitized}.srm`;
}

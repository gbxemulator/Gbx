...
function setupGameSelector() {
    ...
    select.addEventListener('mousedown', (e) => {
        if (GBX.isRunning) e.preventDefault();
    });
...
/** Met à jour l'affichage selon GBX_saveFolder */
...
/**
 * Restaure le dossier enregistré au démarrage (si permission déjà accordée).
 * Appelé après updateAuthUI quand l'utilisateur est connecté.
 */
async function restoreSaveFolder() {
    if (!GBX.user || !window.showDirectoryPicker) return;
    ...
    try {
        ...
        // Si 'prompt', l'utilisateur devra reclicker sur le bouton — on ne force pas
    } catch(e) { /* handle obsolète */ }
}

/**
 * Initialise le bouton "Choisir un dossier de réception".
 */

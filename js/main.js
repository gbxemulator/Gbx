...
async function launchGame(game) {
    ...
    // ── Chargement du .sav existant avant démarrage ──────────
    window.EJS_externalFiles = undefined;
    if (GBX_saveFolder && game.saveName) {
        try {
            const _fh  = await GBX_saveFolder.getFileHandle(game.saveName);
            const _f   = await _fh.getFile();
            const _buf = await _f.arrayBuffer();
            window.EJS_externalFiles = [{ name: game.saveName, data: new Uint8Array(_buf) }];
            showToast('💾 Sauvegarde "' + game.saveName + '" chargée !', 'success');
        } catch(e) {
            if (e.name !== 'NotFoundError') console.warn('[GBX] Lecture .sav :', e);
        }
    }

    // ── Écriture du .sav après chaque sauvegarde in-game ─────
    window.EJS_onSaveSave = async (data) => {
        ...
        try {
            const fh  = await GBX_saveFolder.getFileHandle(game.saveName, { create: true });
            const wr  = await fh.createWritable();
            await wr.write(data instanceof Uint8Array ? data : new Uint8Array(data));
            await wr.close();
            showToast('💾 Sauvegarde écrite : "' + game.saveName + '"', 'success');
        } catch(e) {
            console.error('[GBX] Écriture .sav :', e);
    ...
    window.EJS_onLoadSave = async () => {
        ...
        try {
            const fh  = await GBX_saveFolder.getFileHandle(game.saveName);

...
function setupKeyboard() {
    // Actions spéciales GBX (non-jeu) — utilisent les clés du customKeys
    const gbxActions = {
        '_menu':  () => {
    ...
    // Retourne l'action (id) associée à un code de touche, selon le mapping actuel
    function getActionForCode(code) {
        const map = GBX.customKeys || GBX_KEYMAP_DEFAULT;
        return Object.entries(map).find(([id, c]) => c === code)?.[0] || null;
    }

    document.addEventListener('keydown', (e) => {
        // Touche _save quand modal save-confirm ouverte = confirmer
        const saveKey = (GBX.customKeys || GBX_KEYMAP_DEFAULT)['_save'];
        if (e.code === saveKey && !e.repeat) {
        ...
        // Touche mappée sur "Bouton B" quand modal save-confirm ouverte = fermer
        const bKey = (GBX.customKeys || GBX_KEYMAP_DEFAULT)['btn-b'];
        if (e.code === bKey && !e.repeat) {
            const modalEl = document.getElementById('modal-save-confirm');
            if (modalEl?.classList.contains('open')) {
                e.preventDefault();
                e.stopImmediatePropagation();
        ...
        // Vérifier si c'est une action GBX (non-jeu)
        const actionId = getActionForCode(e.code);
        if (actionId && gbxActions[actionId]) {
            e.preventDefault();
            e.stopImmediatePropagation();
            gbxActions[actionId]();
            return;
    ...
    document.addEventListener('keyup', (e) => {
        ...
        const actionId = (GBX.customKeys ? Object.entries(GBX.customKeys).find(([, c]) => c === e.code)?.[0] : null)
                      || (Object.entries(GBX_KEYMAP_DEFAULT).find(([, c]) => c === e.code)?.[0]);
        if (actionId && ['_menu','_save','_fast','_cheat'].includes(actionId)) {
            e.preventDefault();
            e.stopImmediatePropagation();
            return;
        }
...
// Alias pour le nouveau mapping (touche L AZERTY)
function _handleSaveKeyL() {
    _handleSaveKeyG();
}
...
/* ============================================================
   PC CONTROLS — Volume + Taille écran
   ============================================================ */
function setupPcControls() {
    // Cacher les contrôles PC sur mobile
    const pcControls = document.getElementById('pc-controls');
    if (!pcControls) return;

    function updatePcVisibility() {
        const isMobile = window.matchMedia('(max-width: 480px) and (orientation: portrait)').matches
                      || window.matchMedia('(max-width: 900px) and (orientation: landscape)').matches;
        pcControls.style.display = isMobile ? 'none' : 'flex';
    }
    updatePcVisibility();
    ...
    volBtn?.addEventListener('click', (e) => {
        ...
        volBtn.setAttribute('aria-expanded', String(volOpen));
        screenPanel.hidden = true;
        screenOpen = false;
    });

    // Marquer le dot actif au volume courant
    ...
    let currentVolume = GBX_EMULATOR_CONFIG.volume ?? 0.8;
    updateVolDots(currentVolume);
    ...
    volPanel?.querySelectorAll('.pc-panel-dot[data-volume]').forEach(dot => {
        dot.addEventListener('click', (e) => {
            ...
            currentVolume = val;
            updateVolDots(val);
            // Appliquer le volume à l'émulateur si lancé
            try {
                const audio = document.querySelector('#ejs-container audio') ||
                              document.querySelector('#ejs-container video');
                if (audio) audio.volume = val;
                if (GBX.emulator?.gameManager?.functions?.setVolume) {
                    GBX.emulator.gameManager.functions.setVolume(val);
                }
            } catch(_) {}
            volOpen = false;
    ...
    screenBtn?.addEventListener('click', (e) => {
        ...
        screenBtn.setAttribute('aria-expanded', String(screenOpen));
        volPanel.hidden = true;
        volOpen = false;
    });
    ...
    function applyScreenSize(size) {
        ...
        if (!screenFrame || !screenContainer) return;

        // Retirer plein écran si actif
        if (document.fullscreenElement) {
            document.exitFullscreen().catch(() => {});
        }

        screenFrame.style.width = '';
        screenFrame.style.margin = '';
        ...
        if (size === 'default') {
            ...
            screenContainer.style.width = Math.round(vw * 0.30) + 'px';
            screenContainer.style.height = '';
        } else if (size === '60') {
            const vw = window.innerWidth;
            screenContainer.style.width = Math.round(vw * 0.60) + 'px';
            screenContainer.style.height = '';
            ...
            if (ejsContainer?.requestFullscreen) {
                ejsContainer.requestFullscreen().catch(() => {
                    // Fallback: agrandir l'écran au max
        ...
        currentSize = size;
        screenPanel?.querySelectorAll('.pc-panel-dot--screen').forEach(d => {
    ...
    screenPanel?.querySelectorAll('.pc-panel-dot--screen').forEach(dot => {
        dot.addEventListener('click', (e) => {
            e.stopPropagation();
    ...
    // Échap = revenir à default
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && currentSize !== 'default' && !isModalOpen()) {
            applyScreenSize('default');
        }
    });
    ...
    document.addEventListener('click', () => {
        if (volOpen)    { volPanel.hidden = true;    volOpen = false; }
        if (screenOpen) { screenPanel.hidden = true; screenOpen = false; }
    });
}
...
// Stockage du mapping personnalisé : { keyCode → { ejsIndex, btnId } }
// On utilise localStorage pour persistance sans base de données supplémentaire
// (le mapping est par navigateur/profil, lié au localStorage)
const GBX_KEYMAP_KEY = 'gbx-keymap';

// Définition des actions mappables
...
function keyCodeToLabel(code) {
    const labels = {
        ...
        'Space':'Espace','Enter':'Entrée','Backspace':'Ret.arr','Tab':'Tab',
        'ArrowUp':'↑','ArrowDown':'↓','ArrowLeft':'←','ArrowRight':'→',
        'Semicolon':';/M','Comma':',','Period':'.','Slash':'/','Backslash':'\\',
...
// Applique le keymap chargé sur AZERTY_MAP et gbxKeys
function applyKeyMap(map) {
    // Réinitialiser AZERTY_MAP avec le mapping actuel
    // D'abord, vider toutes les entrées existantes sauf arrows
    ['KeyQ','KeyW','KeyA','KeyS','KeyZ','KeyX',
     'KeyE','KeyR','KeyT','KeyY','KeyU','KeyI','KeyO','KeyP',
     'KeyD','KeyF','KeyG','KeyH','KeyJ','KeyK','KeyL','KeyM','KeyN',
     'KeyB','KeyC','KeyV',
     'Semicolon','Comma','Period','Slash','Quote','Backquote',
     'Digit1','Digit2','Digit3','Digit4','Digit5','Digit6','Digit7','Digit8','Digit9','Digit0',
    ].forEach(k => { delete AZERTY_MAP[k]; });

    // Reconstruire AZERTY_MAP pour les boutons de jeu
    ...
    // gbxKeys sont gérés dans setupKeyboard via les codes stockés dans GBX.customKeys
    GBX.customKeys = map;
}
...
function buildKeyMapGrid() {
    ...
    // Listeners pour remapper
    grid.querySelectorAll('.keymap-key-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            // Passer en mode "en attente de touche"
            ...
            const capture = (e) => {
                ...
                // Vérifier conflit
                const conflict = Object.entries(map).find(([k, v]) => v === e.code && k !== actionId);
                if (conflict) {
                    showToast(`⚠️ Touche déjà utilisée pour "${GBX_KEYMAP_ACTIONS.find(a => a.id === conflict[0])?.label}"`, 'error');
                    btn.classList.remove('waiting');
                    btn.textContent = keyCodeToLabel(btn.dataset.current);
                    document.removeEventListener('keydown', capture, true);
                    return;
...
function setupKeyMapping() {
    // Appliquer le mapping sauvegardé au démarrage
    const map = loadKeyMap();
    applyKeyMap(map);
    GBX.customKeys = map;
    ...
    document.getElementById('btn-keymap-reset')?.addEventListener('click', () => {
        ...
        GBX.customKeys = { ...GBX_KEYMAP_DEFAULT };
        buildKeyMapGrid();
        showToast('↩ Mapping réinitialisé', 'success');
    });
}

// openModal est étendu via setupKeyMapping (appel de buildKeyMapGrid via btn-touches)

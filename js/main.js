...
function _renderFavGame() {
    ...
    if (GBX.user.fav_game) {
        // Trouver le jeu dans GBX_GAMES pour avoir son type
        const game = GBX_GAMES?.find(g => g.name === GBX.user.fav_game || g.id === GBX.user.fav_game);
...
function _buildFavGameSelect() {
    ...
    (GBX_GAMES || []).forEach(g => {
        const opt = document.createElement('option');
        opt.value = g.name;
        opt.textContent = (CONSOLE_BADGE_LABELS[g.type?.toLowerCase()] || g.type?.toUpperCase() || '') + '  ' + g.name;
        if (g.name === GBX.user.fav_game) opt.selected = true;
        sel.appendChild(opt);
    });

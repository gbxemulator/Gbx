...
/* ============================================================
   ...
   ============================================================
   Permet à l'utilisateur connecté de déplacer (drag 1 doigt)
   et redimensionner (pinch 2 doigts) les groupes de boutons.
   La disposition est sauvegardée dans Supabase (colonne
   controller_layout de la table users) ou en localStorage
   pour les non-connectés (non utilisé — connectés uniquement).
   ============================================================ */

// ── Éléments éditables et leurs limites de taille ────────────
const GBX_LAYOUT_ELEMENTS = [
    { id: 'dpad',             minScale: 0.7, maxScale: 1.5 },
    { id: 'ab-buttons',       minScale: 0.7, maxScale: 1.5 },
    { id: 'btn-l',            minScale: 0.7, maxScale: 1.5 },
    { id: 'btn-r',            minScale: 0.7, maxScale: 1.5 },
    { id: 'controls-center',  minScale: 0.7, maxScale: 1.5 },
    { id: 'btn-start',        minScale: 0.7, maxScale: 1.5 },
];

// ── Layout par défaut (valeurs CSS actuelles en %) ───────────
const GBX_LAYOUT_DEFAULT = {
    landscape: {
        dpad:            { x: 0.7,  y: 55,  scale: 1 },   // left%vw, top%vh
        'ab-buttons':    { x: 91,   y: 55,  scale: 1 },   // right depuis droite → on stocke left
        'btn-l':         { x: 2.5,  y: 6,   scale: 1 },
        'btn-r':         { x: 90,   y: 6,   scale: 1 },
        'controls-center':{ x: 1.4, y: 88,  scale: 1 },
        'btn-start':     { x: 88,   y: 88,  scale: 1 },
    },
    portrait: {
        dpad:            { x: 2,    y: 68,  scale: 1 },
        'ab-buttons':    { x: 72,   y: 68,  scale: 1 },
        'btn-l':         { x: 2,    y: 42,  scale: 1 },
        'btn-r':         { x: 85,   y: 42,  scale: 1 },
        'controls-center':{ x: 50,  y: 88,  scale: 1 },
        'btn-start':     { x: 60,   y: 88,  scale: 1 },
    },
};
...
const _layoutEdit = {
    ...
    snapshot: null,      // sauvegarde avant édition
    current:  null,      // layout en cours d'édition
};
...
// ── Charger le layout sauvegardé (Supabase) ───────────────────
async function _loadLayout() {
    ...
    try {
        const { data } = await _supa.from('users')
            ...
            .eq('id', GBX.user.id)
            .single();
        return data?.controller_layout ? JSON.parse(data.controller_layout) : null;
    } catch (_) { return null; }
}
...
async function _saveLayout(layout) {
    ...
    await _supa.from('users')
        .update({ controller_layout: JSON.stringify(layout) })
        .eq('id', GBX.user.id);
}

// ── Appliquer un layout aux éléments du DOM ──────────────────
function _applyLayout(layout, mode) {
    const modeLayout = layout?.[mode];
    if (!modeLayout) return;
    GBX_LAYOUT_ELEMENTS.forEach(({ id }) => {
        const el = document.getElementById(id);
        const pos = modeLayout[id];
        if (!el || !pos) return;
...
// ── Lire la position actuelle d'un élément en % viewport ─────
function _readElementPos(el) {
    const r = el.getBoundingClientRect();
    const vw = window.innerWidth;
    const vh = window.innerHeight;
    // Lire le scale depuis le transform inline
    const styleScale = el.style.transform?.match(/scale\(([\d.]+)\)/);
    const scale = styleScale ? parseFloat(styleScale[1]) : 1;
    return {
        x: (r.left / vw) * 100,
        y: (r.top  / vh) * 100,
        scale,
...
// ── Prendre un snapshot du layout actuel depuis le DOM ────────
function _snapshotLayout(mode) {
    const snap = {};
    GBX_LAYOUT_ELEMENTS.forEach(({ id }) => {
        const el = document.getElementById(id);
...
async function startLayoutEdit() {
    ...
    // Charger le layout sauvegardé ou utiliser le snapshot actuel
    const saved = await _loadLayout();
    _layoutEdit.current  = saved || _snapshotLayout(_layoutEdit.mode);
    _layoutEdit.snapshot = JSON.parse(JSON.stringify(_layoutEdit.current));

    // Appliquer le layout chargé
    _applyLayout(_layoutEdit.current, _layoutEdit.mode);

    // Activer l'overlay
    document.getElementById('layout-edit-overlay')?.classList.add('active');

    // Activer la surbrillance + les listeners drag/pinch
...
function _attachDragPinch(el, id, minScale, maxScale) {
    ...
    function onTouchMove(e) {
        ...
        if (e.touches.length === 1 && startTouches.length === 1) {
            ...
            // Mettre à jour le layout courant
            if (!_layoutEdit.current[_layoutEdit.mode]) _layoutEdit.current[_layoutEdit.mode] = {};
            _layoutEdit.current[_layoutEdit.mode][id] = {
                ..._layoutEdit.current[_layoutEdit.mode][id],
                x: newX, y: newY,
            };
            ...
            let newScale = startScale * (distNow / distStart);
    ...
    function onTouchEnd(e) {
        if (e.touches.length === 0) isDragging = false;
            ...
            // Re-initialiser si on passe de 2 à 1 doigt
            startTouches = Array.from(e.touches);
            startRect    = el.getBoundingClientRect();
        }
    }
...
function _detachDragPinch(el) {
    const listeners = _dragListeners.get(el);
    if (!listeners) return;
    el.removeEventListener('touchstart',  listeners.onTouchStart);
    el.removeEventListener('touchmove',   listeners.onTouchMove);
    el.removeEventListener('touchend',    listeners.onTouchEnd);
    el.removeEventListener('touchcancel', listeners.onTouchEnd);
    _dragListeners.delete(el);
}

// ── Initialisation des boutons de l'overlay ───────────────────
function setupLayoutEditor() {
    // Sauvegarder
    document.getElementById('layout-btn-save')?.addEventListener('click', async () => {
        const full = (await _loadLayout()) || {};
        full[_layoutEdit.mode] = _layoutEdit.current[_layoutEdit.mode];
        await _saveLayout(full);
        _stopLayoutEdit();
        showToast('✅ Disposition sauvegardée !', 'success');
    });

    // Annuler — revenir au snapshot d'avant
    document.getElementById('layout-btn-cancel')?.addEventListener('click', () => {
        _applyLayout(_layoutEdit.snapshot, _layoutEdit.mode);
        _layoutEdit.current = JSON.parse(JSON.stringify(_layoutEdit.snapshot));
        _stopLayoutEdit();
        showToast('Modifications annulées.', 'info');
    });

    // Défaut — remettre les valeurs par défaut
    document.getElementById('layout-btn-reset')?.addEventListener('click', async () => {
        _applyLayout(GBX_LAYOUT_DEFAULT, _layoutEdit.mode);
        _layoutEdit.current[_layoutEdit.mode] = JSON.parse(
            JSON.stringify(GBX_LAYOUT_DEFAULT[_layoutEdit.mode])
        );
        // Sauvegarder immédiatement le reset
        const full = (await _loadLayout()) || {};
        full[_layoutEdit.mode] = GBX_LAYOUT_DEFAULT[_layoutEdit.mode];
        await _saveLayout(full);
        _stopLayoutEdit();
        showToast('↩ Disposition par défaut restaurée.', 'success');
    });

    // Appliquer le layout sauvegardé au chargement de la page
    _loadLayout().then(saved => {
        if (!saved) return;
        const mode = _layoutMode();
        if (saved[mode]) _applyLayout(saved, mode);
    });

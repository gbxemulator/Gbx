...
function _captureState() {
    ...
    // Méthode 1 : gambatte (GB/GBC) — API EmulatorJS haut niveau
    try {
        const state = gm.Module?.EmulatorJSGetState?.();
        if (state && state.length > 0) return state;
    } catch(_) {}

    // Méthode 2 : mgba (GBA) — lire directement le heap WASM
    // _save_state_info() retourne la taille du state en mémoire
    // On alloue un buffer WASM, on appelle _cmd_save_state(),
    // puis on copie le heap depuis le pointeur interne de RetroArch.
    // En pratique : on lit le heap HEAPU8 au pointeur que _save_state_info
    // indique être la taille, en cherchant le ptr via l'ABI RetroArch.
    try {
        const M = EJS.compression?.EJS?.Module;
        if (!M) return null;

        const size = M._save_state_info();
        if (!size || size <= 0 || size > 64_000_000) return null;

        // Allouer un buffer dans le heap WASM
        ...
        // Déclencher la sérialisation : _cmd_save_state écrit dans le buffer
        // interne de RetroArch. On doit passer par retro_serialize si dispo,
        // sinon on lit depuis le heap après _cmd_save_state.
        // On cherche le symbole _core_serialize dans le Module
        let ok = false;
        if (typeof M._retro_serialize === 'function') {
            ok = M._retro_serialize(ptr, size) !== 0;
        } else {
            // Fallback : utiliser ccall si disponible
            try {
                ok = M.ccall('retro_serialize', 'number', ['number','number'], [ptr, size]) !== 0;
            } catch(_) {}
        }
        ...
        // Copier hors du heap WASM avant que _free ne le recycle
        const copy = new Uint8Array(size);
        copy.set(M.HEAPU8.subarray(ptr, ptr + size));
        M._free(ptr);

        if (copy.some(b => b !== 0)) return copy;
        M._free?.(ptr);
        return null;
    } catch(e) { console.warn('[GBX] captureState GBA:', e); return null; }
}
...
function _applyState(b64) {
    ...
    // Méthode 1 : gambatte (GB/GBC) — écrire dans le FS WASM puis loadState
    try {
    ...
    // Méthode 2 : mgba (GBA) — écrire directement dans le heap WASM
    // puis appeler retro_unserialize
    const M = EJS.compression?.EJS?.Module;
    if (!M) throw new Error('Module WASM non disponible');

    const ptr = M._malloc(arr.length);
    if (!ptr) throw new Error('malloc échoué');
    M.HEAPU8.set(arr, ptr);

    let ok = false;
    if (typeof M._retro_unserialize === 'function') {
        ok = M._retro_unserialize(ptr, arr.length) !== 0;
    } else {
        try {
            ok = M.ccall('retro_unserialize', 'number', ['number','number'], [ptr, arr.length]) !== 0;
        } catch(_) {}
    }
    M._free(ptr);

    if (!ok) throw new Error('retro_unserialize a échoué');
}
...
async function _restoreIngameSave(gameId) {
    ...
    try {
        ...
        if (saveRow?.data) {
            ...
            console.info('[GBX] Save Supabase chargee, en attente injection FS');
        }

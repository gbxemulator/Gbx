...
function _createBubbleEl(user, index, total, color) {
    ...
    if (user.avatar && user.avatar !== 'assets/avatar-default.png') {
        ...
        fallback.textContent = user.pseudo ? user.pseudo[0].toUpperCase() : '?';
...
function _spawnParticles(wrap, color) {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        const dist  = _randomBetween(40, 80);
...
function _closeBubbleProfile(card, overlay, bubbleWrap) {
    ...
    setTimeout(() => {
        ...
        // Réapparaître la bulle après 3s
        if (bubbleWrap) {
            setTimeout(() => {
                bubbleWrap.style.display = '';
                bubbleWrap.classList.remove('popping');
                bubbleWrap.style.animationPlayState = '';
                // Reset position aléatoire
                const canvas = document.getElementById('bubble-canvas');
                if (canvas) {
                    const newDelay = _randomBetween(0.5, 3);
                    bubbleWrap.style.animationDelay = '-' + newDelay + 's';
                }
            }, 3000);
        }
    }, 250);

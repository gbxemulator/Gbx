...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    setupLayoutEditor();      // Disposition boutons mobile/landscape
    ...
    if (session) {
        ...
        if (profile) {
            ...
            // Appliquer le layout sauvegardé dès que l'user est connu
            _applyLayoutOnLoad();
        }
    }
    updateAuthUI();
...
function setupGameControls() {
    ...
    Object.entries(buttonMap).forEach(([btnId, ejsRealIndex]) => {
        ...
        el.addEventListener('touchend', (e) => {
            pressButton(el, ejsRealIndex, false);
        }, { passive: true });
...
function pressButton(el, ejsRealIndex, isDown) {
    ...
    if (_layoutEdit?.active) return; // bloquer pendant le mode édition
...
function setupExtraButtons() {
    ...
    // Bouton Touches → modal keymap (PC uniquement)
    ...
    // Bouton Boutons → mode édition layout (mobile/landscape uniquement)
    document.getElementById('btn-boutons')?.addEventListener('click', () => {
        toggleSidebar(false);
        startLayoutEdit();
    });
}
...
/* ============================================================
   LAYOUT EDITOR — Disposition des boutons (mobile + landscape)
   ============================================================
   Drag 1 doigt → déplacer. Pinch 2 doigts → redimensionner.
   Sauvegarde dans Supabase (colonne controller_layout, table users).
   Visible uniquement pour les utilisateurs connectés.

   Portrait  : dpad, ab-buttons, start-select (SELECT+START groupés, 134×18px)
   Landscape : dpad, ab-buttons, btn-l, btn-r, btn-select, btn-start
               (tous déjà fixed — btn-start et controls-center sont
               ancêtre/descendant, on ne les met JAMAIS ensemble)

   PRINCIPE : on ne touche PAS aux positions au démarrage.
   L'élément est figé en fixed uniquement au premier touchstart,
   à partir de getBoundingClientRect() — position réelle et exacte.
   ============================================================ */

// ── Éléments éditables selon le mode ─────────────────────────
// En portrait : SELECT et START sont groupés dans #controls-center
// En landscape : ils sont indépendants (fixed séparément), L et R aussi
const GBX_LAYOUT_ELEMENTS = {
    // portrait : dpad et ab-buttons sont en position:relative dans le flux.
    // start-select (134×18px) est le vrai conteneur SELECT+START groupés.
    // controls-center fait 338px de haut (flex container) — ne pas l'utiliser.
    portrait: [
        { id: 'dpad' },
        { id: 'ab-buttons' },
        { id: 'start-select' },
    ],
    // landscape : tous déjà en position:fixed via CSS.
    // btn-start est un descendant de controls-center — ne jamais mettre
    // les deux dans la liste (conflit touch capture ancestor/descendant).
    // On édite btn-select et btn-start directement.
    landscape: [
        { id: 'dpad' },
        { id: 'ab-buttons' },
        { id: 'btn-l' },
...
// ── Retourner la liste d'éléments pour le mode actuel ────────
function _layoutElements() {
    return GBX_LAYOUT_ELEMENTS[_layoutEdit.mode] || [];
}

// ── État interne ──────────────────────────────────────────────
const _layoutEdit = {
    active:   false,
    mode:     null,    // 'landscape' | 'portrait'
    snapshot: null,    // styles inline avant édition (pour Annuler)
    current:  null,    // positions en cours (pour Sauvegarder)
};

// Listeners drag/pinch par élément
const _dragListeners = new Map();
...
// ── Appliquer une position sauvegardée sur un élément ─────────
function _applyPos(el, pos) {
    // absolute dans #gameboy — position:fixed est cassé par le transform CSS de #gameboy
...
// ── Réinitialiser les styles inline d'un élément ─────────────
function _clearInlinePos(el) {
    el.style.position        = '';
    el.style.left            = '';
    el.style.top             = '';
    el.style.right           = '';
...
// ── Charger le layout depuis Supabase ─────────────────────────
async function _loadLayout() {
    if (!GBX.user) return null;
    try {
        const { data } = await _supa.from('users')
            .select('controller_layout')
            .eq('id', GBX.user.id)
            .single();
        if (!data?.controller_layout) return null;
        return JSON.parse(data.controller_layout);
    } catch (_) { return null; }
}

// ── Sauvegarder le layout dans Supabase ──────────────────────
async function _saveLayout(modeData) {
    if (!GBX.user) return;
    const existing = (await _loadLayout()) || {};
    existing[_layoutEdit.mode] = modeData;
    await _supa.from('users')
        .update({ controller_layout: JSON.stringify(existing) })
        .eq('id', GBX.user.id);
}

// ── Appliquer un layout chargé aux éléments du DOM ───────────
// Appelé au chargement et au changement d'orientation.
// N'est jamais appelé si aucun layout n'est sauvegardé.
function _applyLayoutToDOM(modeData, mode) {
    if (!modeData) return;
    const elements = GBX_LAYOUT_ELEMENTS[mode] || [];
    elements.forEach(({ id }) => {
        const el  = document.getElementById(id);
        const pos = modeData[id];
        if (el && pos) _applyPos(el, pos);
    });
}

// ── Démarrer l'édition ────────────────────────────────────────
async function startLayoutEdit() {
    if (_layoutEdit.active) return;
    _layoutEdit.active = true;
    _layoutEdit.mode   = _layoutMode();

    // Mémoriser les styles inline existants pour le bouton Annuler.
    // On ne touche PAS aux positions — le CSS continue de gérer
    // l'affichage exactement comme avant.
    _layoutEdit.snapshot = { inlineStyles: {}, controlsMainHeight: null };
    _layoutEdit.current  = {};

    _layoutElements().forEach(({ id }) => {
        const el = document.getElementById(id);
        if (!el) return;
        _layoutEdit.snapshot.inlineStyles[id] = {
    ...
    // En landscape, retirer disabled sur btn-l et btn-r (disabled sur jeux GB/GBC)
    // pour que les touch events soient bien reçus
    if (_layoutEdit.mode === 'landscape') {
        ['btn-l', 'btn-r'].forEach(id => {
            const el = document.getElementById(id);
            if (!el) return;
            _layoutEdit.snapshot.inlineStyles[id] = _layoutEdit.snapshot.inlineStyles[id] || {};
            _layoutEdit.snapshot.inlineStyles[id]._wasDisabled = el.disabled;
            el.disabled = false;
        });
    }

    // En portrait, figer controls-main en hauteur pour éviter l'effondrement
    // du flux quand dpad et ab-buttons passent en position:fixed
    if (_layoutEdit.mode === 'portrait') {
        const cm = document.getElementById('controls-main');
        if (cm) {
            const h = cm.getBoundingClientRect().height;
            _layoutEdit.snapshot.controlsMainHeight = cm.style.minHeight;
            cm.style.minHeight = h + 'px';
        }
    }

    // Activer l'overlay + surbrillance
    document.getElementById('layout-edit-overlay')?.classList.add('active');
    _layoutElements().forEach(({ id }) => {
        const el = document.getElementById(id);
        if (!el) return;
...
// ── Stopper l'édition ─────────────────────────────────────────
function _stopLayoutEdit() {
    _layoutEdit.active = false;
    document.getElementById('layout-edit-overlay')?.classList.remove('active');
    _layoutElements().forEach(({ id }) => {
        const el = document.getElementById(id);
        if (!el) return;
    ...
    // Restaurer disabled sur btn-l et btn-r
    ['btn-l', 'btn-r'].forEach(id => {
        const el  = document.getElementById(id);
        const was = _layoutEdit.snapshot?.inlineStyles?.[id]?._wasDisabled;
        if (el && was) el.disabled = true;
    });

    // Restaurer controls-main
    const cm = document.getElementById('controls-main');
    if (cm && _layoutEdit.snapshot?.controlsMainHeight !== null) {
        cm.style.minHeight = _layoutEdit.snapshot.controlsMainHeight || '';
    }
}

// ── Restaurer les styles inline d'avant l'édition ────────────
function _restoreInlineStyles() {
    if (!_layoutEdit.snapshot?.inlineStyles) return;
    _layoutElements().forEach(({ id }) => {
        const el = document.getElementById(id);
        const s  = _layoutEdit.snapshot.inlineStyles[id];
        if (!el || !s) return;
        // Remettre dans le parent original si téléporté
        if (s._origParent && el.parentElement !== s._origParent) {
            if (s._origNextSib) s._origParent.insertBefore(el, s._origNextSib);
            else s._origParent.appendChild(el);
        }
...
// ── Drag + Pinch sur un élément ───────────────────────────────
// L'élément est figé en position:fixed uniquement au premier touchstart,
// à partir de getBoundingClientRect() — sa vraie position visuelle.
function _attachDrag(el, id) {
    let startTouch  = null;
    let startRect   = null;
    let fixedAtStart = false;

    function onTouchStart(e) {
        e.stopPropagation();
        // Ignorer les gestes multi-doigts
        if (e.touches.length !== 1) return;

        // Figer en absolute dans #gameboy au premier touch.
        // On soustrait l'origine de #gameboy car getBoundingClientRect()
        // retourne des coords viewport, pas relatives à gameboy.
        if (!fixedAtStart) {
            const gb = document.getElementById('gameboy');
            // 1. Mémoriser position viewport AVANT de toucher au DOM
            const rBefore = el.getBoundingClientRect();
            const viewportLeft = rBefore.left;
            const viewportTop  = rBefore.top;
            // 2. Mémoriser le parent original pour restauration (Annuler)
            if (!_layoutEdit.snapshot.inlineStyles[id]) _layoutEdit.snapshot.inlineStyles[id] = {};
            _layoutEdit.snapshot.inlineStyles[id]._origParent  = el.parentElement;
            _layoutEdit.snapshot.inlineStyles[id]._origNextSib = el.nextSibling;
            // 3. Téléporter dans #gameboy (peut changer le flux et déplacer gameboy)
            if (gb && el.parentElement !== gb) gb.appendChild(el);
            // 4. Relire gameboy APRES le appendChild (le flux a pu changer)
            const gr = gb ? gb.getBoundingClientRect() : { left: 0, top: 0 };
            // 5. Calculer la position relative à gameboy
            const x = viewportLeft - gr.left;
            const y = viewportTop  - gr.top;
            el.style.position        = 'absolute';
            el.style.left            = x + 'px';
            el.style.top             = y + 'px';
            el.style.right           = '';
    ...
    function onTouchMove(e) {
        e.stopPropagation();
        if (!fixedAtStart || !startTouch || e.touches.length !== 1) return;

        const gb  = document.getElementById('gameboy');
        const gbR = gb ? gb.getBoundingClientRect() : { left: 0, top: 0, width: window.innerWidth, height: window.innerHeight };
        const dx  = e.touches[0].clientX - startTouch.x;
        const dy  = e.touches[0].clientY - startTouch.y;
        // startRect est en coords viewport → convertir en coords gameboy
        const startX = startRect.left - gbR.left;
        const startY = startRect.top  - gbR.top;
        let newX  = startX + dx;
        let newY  = startY + dy;
        // Limiter aux dimensions de #gameboy
        newX = Math.max(0, Math.min(gbR.width  - startRect.width,  newX));
        newY = Math.max(0, Math.min(gbR.height - startRect.height, newY));
        el.style.left = newX + 'px';
        el.style.top  = newY + 'px';
        _layoutEdit.current[id] = { x: newX, y: newY };
    }
    ...
    _dragListeners.set(el, { onTouchStart, onTouchMove, onTouchEnd });
}


function _detachDrag(el) {
    const L = _dragListeners.get(el);
    if (!L) return;
    el.removeEventListener('touchstart',  L.onTouchStart, { capture: true });
    el.removeEventListener('touchmove',   L.onTouchMove,  { capture: true });
    el.removeEventListener('touchend',    L.onTouchEnd,   { capture: true });
    el.removeEventListener('touchcancel', L.onTouchEnd,   { capture: true });
    _dragListeners.delete(el);
}

// ── Charger et appliquer le layout sauvegardé (appelé après auth) ──
async function _applyLayoutOnLoad() {
    if (!GBX.user) return;
    const saved = await _loadLayout();
    if (!saved) return;
    const mode = _layoutMode();
    if (saved[mode]) _applyLayoutToDOM(saved[mode], mode);
}

// ── Init : boutons overlay + ré-application sur orientationchange ─
function setupLayoutEditor() {

    // Bouton Sauvegarder
    document.getElementById('layout-btn-save')?.addEventListener('click', async () => {
        await _saveLayout(_layoutEdit.current);
        _stopLayoutEdit();
        showToast('✅ Disposition sauvegardée !', 'success');
    });

    // Bouton Annuler
    document.getElementById('layout-btn-cancel')?.addEventListener('click', () => {
        _restoreInlineStyles();
        _stopLayoutEdit();
        showToast('Modifications annulées.', 'info');
    });

    // Bouton Défaut — efface le layout de ce mode et remet le CSS
    document.getElementById('layout-btn-reset')?.addEventListener('click', async () => {
        if (GBX.user) {
            const existing = (await _loadLayout()) || {};
            delete existing[_layoutEdit.mode];
            await _supa.from('users')
                .update({ controller_layout: JSON.stringify(existing) })
                .eq('id', GBX.user.id);
        }
    ...
    // Ré-appliquer si l'orientation change
    window.addEventListener('orientationchange', () => {
        setTimeout(async () => {
            if (!GBX.user) return;
            const saved = await _loadLayout();
            if (!saved) return;
            const mode = _layoutMode();
            // Clear tous les éléments des deux modes avant d'appliquer le nouveau
            ['portrait', 'landscape'].forEach(m => {
                (GBX_LAYOUT_ELEMENTS[m] || []).forEach(({ id }) => {
                    const el = document.getElementById(id);
                    if (el) _clearInlinePos(el);
                });
            });
            if (saved[mode]) _applyLayoutToDOM(saved[mode], mode);
        }, 350);

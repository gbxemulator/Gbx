...
async function _restoreIngameSave(gameId) {
    ...
    if (!saveArr) {
        // Pas de save — démarrer l'auto-save directement après le lancement
        setTimeout(() => startAutoSave(), 10_000);
        return;
    }
...
function startAutoSave() {
    ...
    _autoSaveTimer = setInterval(async () => {
        ...
        try {
            ...
            // Protection régression : ne jamais écraser une save Supabase
            // avec moins de données (micro-save automatique du jeu au menu)
            const { data: existing } = await _supa
                .from('saves').select('data')
                .eq('user_id', GBX.user.id)
                .eq('game_id', GBX.currentGame.id)
                .maybeSingle();
            if (existing?.data) {
                const existArr = new Uint8Array(atob(existing.data).split('').map(c => c.charCodeAt(0)));
                const existNonFF = Array.from(existArr).filter(b => b !== 0xFF).length;
                if (nonFF < existNonFF) return; // régression — on skip
            }

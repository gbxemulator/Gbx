...
async function launchGame(game) {
    ...
    // ── Récupérer la save depuis Supabase AVANT de lancer le core ──
    window._gbxPendingSave = null;
    window.EJS_oldSave     = undefined;
    if (GBX.user) {
        try {
            const { data } = await _supa
                ...
                .maybeSingle();
            if (data?.save_data) {
                window._gbxPendingSave = data.save_data;
                window.EJS_oldSave     = data.save_data;
            }
        } catch(e) {}
    }
    ...
    // Callbacks save
    window.EJS_onSaveSave = GBX.user ? (data) => {
        uploadSaveToServer(GBX.currentGame, data);
    } : undefined;
    ...
    window.EJS_ready = () => {
        ...
        // ── Injecter la SRAM dans le FS dès que l'émulateur est prêt ──
        if (window._gbxPendingSave) {
            try {
                const base64  = window._gbxPendingSave.split(',')[1];
                ...
                for (let i = 0; i < parts.length - 1; i++) {
                    cur += '/' + parts[i];
                    try { GBX.emulator.gameManager.FS.mkdir(cur); } catch(e) {}
                }

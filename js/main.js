...
function showEjectConfirm() {
    ...
    backdrop.innerHTML = `
        <div style="background:#1e1e2e;border-radius:16px;padding:28px 32px;max-width:320px;width:90%;text-align:center;box-shadow:0 8px 32px rgba(0,0,0,0.5);">
            ...
            <h3 style="color:#fff;margin:0 0 22px;font-size:1.1rem">Éjecter le jeu ?</h3>
            <div style="display:flex;gap:10px;justify-content:center">
                ...
                <button id="eject-confirm" style="flex:1;padding:10px;border-radius:10px;border:none;cursor:pointer;background:#e63946;color:#fff;font-size:0.9rem;font-weight:700">🚪 Éjecter</button>
            </div>
        </div>
    `;
...
/* ============================================================
   ...
   ============================================================
   Le joueur choisit un dossier de réception pour ses .sav.
   Si l'API showDirectoryPicker est disponible (PC/Chromium), on l'utilise.
   Sinon on bascule sur un simple input[type=file] pour mobile.
   La save est écrite dans l'IndexedDB d'EmulatorJS (/data/saves)
   AVANT le prochain lancement du jeu.
   ============================================================ */
...
// Dossier de réception sélectionné par l'utilisateur (FileSystemDirectoryHandle ou null)
let GBX_saveFolder = null;

/**
 * Initialise la section "Mes Sauvegardes" dans la sidebar.
 */
function setupSavesUI() {
    ...
    if (!btnFolder || !btnLoad) return;

    btnFolder.addEventListener('click', async () => {
        // API showDirectoryPicker (Chromium Desktop/Android)
        if (window.showDirectoryPicker) {
            try {
                GBX_saveFolder = await window.showDirectoryPicker({ mode: 'readwrite' });
                ...
                showToast('✅ Dossier "' + GBX_saveFolder.name + '" sélectionné !', 'success');
            } catch(e) {
                if (e.name !== 'AbortError') showToast('❌ Impossible d\'ouvrir le dossier.', 'error');
            }
        } else {
            // Fallback mobile : on informe simplement
    ...
    btnLoad.addEventListener('click', () => {
        ...
        const targetGame = GBX.currentGame;
        if (!targetGame) {
            showToast('Lance d\'abord un jeu avant de charger une save.', 'error');
            return;
        }
        ...
        input.addEventListener('change', async () => {
            ...
            reader.onload = async (ev) => {
                ...
                if (bytes.length === 0 || bytes.length > 131072) {
                    showToast('❌ Fichier invalide (taille incorrecte).', 'error');
                    return;
                }

                const path = getSavePath(targetGame);

                // 1. Écriture dans l'IDB
                await writeToIndexedDB(path, bytes);

                // 2. Si le jeu est en cours, injection directe dans le FS mémoire
                if (GBX.isRunning && GBX.currentGame?.id === targetGame.id) {
                    try {
                        ...
                        if (fs) {
                            ...
                            showToast('💾 Save chargée en direct dans ' + targetGame.name + ' !', 'success');
                            return;
                        }
                    } catch(e) {
                        console.warn('[GBX] Injection FS directe échouée:', e);
                    }
                }

                showToast('✅ Save importée — relance le jeu pour jouer !', 'success');
            };

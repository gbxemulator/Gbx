...
function openProfileModal() {
    ...
    if (lastGameEl) {
        if (GBX.user.last_game) {
            lastGameEl.textContent = '🎮 Dernier jeu : ' + GBX.user.last_game;
        } else {
            lastGameEl.textContent = '';
        }
    }
...
function setupPcControls() {
    ...
    let currentVol = GBX_EMULATOR_CONFIG.volume ?? 0.8;
    ...
    function resetAutoClose() {
        if (volOpen || screenOpen) startAutoClose();
    }
    ...
    // Initialiser l'icône sans afficher le panel
    ...
    volPanel?.querySelectorAll('.pc-panel-dot[data-volume]').forEach(dot => {
        dot.addEventListener('click', (e) => {
            ...
            // Appliquer le volume — EmulatorJS attend 0-1
            try {
                // 1) Tous les éléments audio/video dans l'iframe ou le conteneur
                document.querySelectorAll('audio, video').forEach(el => {
                    el.volume = val;
                });
                // 2) API EmulatorJS (gameManager.functions.setVolume attend 0-100)
                const gm = GBX.emulator?.gameManager;
                if (gm?.functions?.setVolume) {
                    gm.functions.setVolume(Math.round(val * 100));
                } else if (gm?.volume !== undefined) {
                    gm.volume = val;
                }
            } catch(_) {}
    ...
    // ── Taille écran ─────────────────────────────────────────────
    // "Petit Full" = screen-frame fixe centré, sans gameboy autour
    function applyScreenSize(size) {
        ...
        // Reset les styles custom sur screen-frame
        if (screenFrame) {
            screenFrame.style.position  = '';
            screenFrame.style.inset     = '';
            screenFrame.style.zIndex    = '';
            screenFrame.style.margin    = '';
            screenFrame.style.borderRadius = '';
        ...
        if (size === 'default') {
            // Rien d'autre — styles réinitialisés ci-dessus
        } else if (size === 'petit-full') {
            // Screen-frame en fixed, centré, prend ~80% de la page
            if (screenFrame) {
                screenFrame.style.position     = 'fixed';
                screenFrame.style.top          = '50%';
                screenFrame.style.left         = '50%';
                screenFrame.style.transform    = 'translate(-50%, -50%)';
                screenFrame.style.width        = '80vw';
                screenFrame.style.height       = '80vh';
                screenFrame.style.zIndex       = '500';
                screenFrame.style.borderRadius = '16px';
                screenFrame.style.margin       = '0';
            }
    ...
    // Dot screen actif par défaut = 'default'
    screenPanel?.querySelectorAll('.pc-screen-dot').forEach(d => {
    ...
    // Échap → retour à la taille par défaut
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && currentSize !== 'default') {
    ...
    // Quitter fullscreen natif → reset

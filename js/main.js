...
/* ============================================================
   11. SAUVEGARDES — SUPABASE (via fichier .srm/.sav)
   ============================================================
   Utilise saveSaveFiles() pour forcer l'écriture du fichier
   de sauvegarde dans le FS Emscripten, puis le lit via FS.readFile.
   Le chargement écrit le fichier dans le FS puis appelle
   loadSaveFiles() pour que le core le prenne en compte.

   3 slots par utilisateur + jeu, stockés dans la table
   `save_states` (Supabase). Les données sont en base64.
...
// ── Lire le fichier .srm depuis le FS Emscripten en base64 ───
function _getSrmBase64() {
    try {
        const gm = GBX.emulator?.gameManager || window.EJS_emulator?.gameManager;
        if (!gm) return null;

        // Forcer l'écriture du fichier .srm dans le FS
        if (typeof gm.functions?.saveSaveFiles === 'function') {
            gm.functions.saveSaveFiles();
        }

        // Lire le chemin du fichier de sauvegarde
        const path = gm.functions?.getSaveFilePath?.();
        if (!path) { console.warn('[GBX] getSaveFilePath indisponible'); return null; }
        ...
        console.warn('[GBX] getSrmBase64:', e);
        return null;
    }
}

// ── Écrire le .srm dans le FS et recharger via le core ───────
function _applySrmBase64(b64) {
    ...
    // Écrire dans le FS Emscripten
    gm.FS.writeFile(path, arr);

    // Demander au core de recharger les saves
    if (typeof gm.functions?.loadSaveFiles === 'function') {

    ...
    if (GBX.user) {
        try {
            const { data } = await _supa
                ...
                .maybeSingle();
            if (data?.save_data) {
                // Convertir base64 → Blob URL pour EJS_externalFiles
                ...
                // Injecter dans le FS avant que le core démarre
                const savePath = getSavePath(game);
                window.EJS_externalFiles = { [savePath]: blobUrl };
                window._gbxPendingSave   = data.save_data; // gardé pour l'autosave
            }
        } catch(e) {}
    }

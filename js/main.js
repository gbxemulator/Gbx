...
// Annule la capture clavier en cours (si une touche était en attente)
let _keyCaptureCancel = null;

function _cancelKeyCapture() {
    if (_keyCaptureCancel) {
        _keyCaptureCancel();
...
function buildKeyMapGrid() {
    ...
    grid.querySelectorAll('.keymap-key-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            // Si ce bouton est déjà en attente, annuler (toggle off)
            if (btn.classList.contains('waiting')) {
                _cancelKeyCapture();
                return;
            }

            // Annuler toute capture précédente avant d'en démarrer une nouvelle
            _cancelKeyCapture();
            ...
            // Clic en dehors → annuler (pas valider)
            const outsideClick = (e) => {
                // Ignorer le clic sur ce bouton lui-même (géré par le listener click)
                if (btn.contains(e.target)) return;
                // Ignorer les clics sur les autres boutons keymap (ils gèrent eux-mêmes)
                if (e.target.classList.contains('keymap-key-btn') ||
                    e.target.classList.contains('keymap-gamepad-btn')) return;
                cancel();
            };
            ...
            _keyCaptureCancel = cancel;
            document.addEventListener('keydown', capture, true);
            // Délai pour ne pas capturer le mousedown du clic courant
            setTimeout(() => document.addEventListener('mousedown', outsideClick, true), 100);
        });
    });
}
        ...
        for (let pi = 0; pi < pads.length; pi++) {
            ...
            pad.buttons.forEach((btn, i) => {
                ...
                for (const [actionId, btnIndex] of Object.entries(map)) {
                    ...
                    if (!action || action.ejsIndex < 0) continue;
...
function buildGamepadGrid() {
    ...
    grid.querySelectorAll('.keymap-gamepad-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            // Toggle : reclique sur le même bouton en attente = annuler
            if (btn.classList.contains('waiting')) {
                gpCancel();
                return;
            }

            // Annuler toute capture manette précédente
            gpCancel();

            // Fermer aussi la capture clavier si ouverte
            _cancelKeyCapture();

            grid.querySelectorAll('.keymap-gamepad-btn').forEach(b => b.classList.remove('waiting'));
            btn.classList.add('waiting');
            btn.textContent = '…';
            ...
            const gpCancel = () => {
                ...
                btn.textContent = gamepadBtnLabel(cur !== 'null' && cur !== '' ? parseInt(cur) : null);
                document.removeEventListener('keydown', gpEscHandler);
                document.removeEventListener('mousedown', gpOutsideClick, true);
                startGamepadPolling();
            };

            // Clic en dehors → annuler
            const gpOutsideClick = (e) => {
                if (btn.contains(e.target)) return;
                if (e.target.classList.contains('keymap-gamepad-btn') ||
                    e.target.classList.contains('keymap-key-btn')) return;
                gpCancel();
            };
            ...
            const captureGamepad = () => {
                ...
                rafCapture = requestAnimationFrame(captureGamepad);
            };

            const gpEscHandler = (e) => {
                if (e.key === 'Escape') gpCancel();
            };

            rafCapture = requestAnimationFrame(captureGamepad);
            document.addEventListener('keydown', gpEscHandler);
            setTimeout(() => document.addEventListener('mousedown', gpOutsideClick, true), 100);
        });
    });
}

...
async function launchGame(game) {
    ...
    // ── Pré-charger la save dans l'IDB AVANT de lancer loader.js ──────────────
    // Gambatte lit sa SRAM depuis l'IDB au démarrage du core.
    // On doit donc écrire AVANT que loader.js soit injecté.
    ...
    if (GBX.user) {
        try {
            const { data } = await _supa
                ...
                .maybeSingle();
            if (data?.save_data) {
                const raw    = data.save_data;
                const base64 = raw.includes(',') ? raw.split(',')[1] : raw;
                const binary = atob(base64);
                const bytes  = new Uint8Array(binary.length);
                for (let i = 0; i < binary.length; i++) bytes[i] = binary.charCodeAt(i);
                // Écrire dans l'IDB d'EmulatorJS — gambatte lira ce fichier au boot
                await writeToIndexedDB(getSavePath(game), bytes);
            }
        } catch(e) {
            console.warn('[GBX] Erreur pré-chargement save :', e);
        }
    }
    ...
    // Callbacks save — EJS_onSaveSave reçoit les bytes quand le jeu sauvegarde
    window.EJS_onSaveSave = (saveData) => {
        if (!GBX.user || !saveData || saveData.length === 0) return;
        uploadSaveToServer(GBX.currentGame, saveData, true).catch(() => {});
    };
    window.EJS_onLoadSave = undefined;
    ...
    window.EJS_ready = () => {
        ...
        if (GBX.user) {
            ...
            GBX._lastSaveHash = null;

            // Toutes les 3s, on regarde si le fichier .srm a changé
            // Si oui → le joueur vient de sauvegarder en jeu → on uploade immédiatement
            GBX.autosaveTimer = setInterval(async () => {
                try {
                    const saveData = readSaveFile(game);
                    if (!saveData) return;

                    // Calcul d'un hash léger (somme des bytes) pour détecter un changement
                    let hash = 0;
                    for (let i = 0; i < saveData.length; i++) hash = (hash + saveData[i] * (i + 1)) >>> 0;

                    if (hash !== GBX._lastSaveHash) {
                        GBX._lastSaveHash = hash;
                        // Le fichier a changé → upload immédiat
                        await uploadSaveToServer(GBX.currentGame, saveData, true);
                        console.info('[GBX] Changement détecté → save uploadée');
                    }
                } catch(e) {}
    ...
    // EJS_onGameStart — colorisation uniquement (la save est déjà dans l'IDB au boot)
...
function showEjectConfirm() {
    ...
    // Vérifier si la save a changé depuis le chargement
    const saveData    = readSaveFile(GBX.currentGame);
    const saveChanged = saveData && GBX._lastSaveHash !== null && (() => {
        let hash = 0;
        for (let i = 0; i < saveData.length; i++) hash = (hash + saveData[i] * (i + 1)) >>> 0;
        return hash !== GBX._lastSaveHash;
    })();
    const neverSaved  = saveData && GBX._lastSaveHash === null;

    let icon, title, msg, confirmLabel, confirmColor;

    if (!saveData) {
        // Pas de fichier valide (vide ou inexistant)
        icon         = '⚠️';
        title        = 'Pense à sauvegarder !';
        msg          = 'Aucune sauvegarde détectée. Si tu éjectes, ta progression sera perdue.';
        confirmLabel = 'Éjecter quand même';
        confirmColor = '#e63946';
    } else if (saveChanged || neverSaved) {
        // Le fichier a changé depuis le chargement → sauvegarde non encore uploadée
        icon         = '💾';
        title        = 'Sauvegarde détectée !';
        msg          = 'Ta progression sera envoyée au serveur avant l\'éjection.';
        confirmLabel = '💾 Sauvegarder & Éjecter';
        confirmColor = '#16a34a';
    } else {
        // Aucun changement → éjection directe sans risque
        icon         = '✅';
        title        = 'Aucun changement';
        msg          = 'Ta sauvegarde est déjà à jour sur le serveur.';
    ...
    backdrop.innerHTML = `
        <div style="background:#1e1e2e;border-radius:16px;padding:28px 32px;max-width:320px;width:90%;text-align:center;box-shadow:0 8px 32px rgba(0,0,0,0.5);">
            <div id="eject-icon" style="font-size:2rem;margin-bottom:8px">${icon}</div>
            <h3 id="eject-title" style="color:#fff;margin:0 0 10px;font-size:1.1rem">${title}</h3>
            <p id="eject-msg" style="color:#aaa;font-size:0.88rem;margin:0 0 22px;line-height:1.5">${msg}</p>
            <div style="display:flex;gap:10px;justify-content:center">
                ...
                <button id="eject-confirm" style="flex:1;padding:10px;border-radius:10px;border:none;cursor:pointer;background:${confirmColor};color:#fff;font-size:0.9rem;font-weight:700">${confirmLabel}</button>
            </div>
        </div>
    `;
    ...
    document.getElementById('eject-confirm').addEventListener('click', async () => {
        const confirmBtn = document.getElementById('eject-confirm');
        const cancelBtn  = document.getElementById('eject-cancel');

        if (saveChanged || neverSaved) {
            // Upload avant d'éjecter
            ...
            try {
                await uploadSaveToServer(GBX.currentGame, saveData, true);
                document.getElementById('eject-icon').textContent  = '✅';
                document.getElementById('eject-title').textContent = 'Sauvegarde réussie !';
                document.getElementById('eject-msg').textContent   = 'Ta progression est bien enregistrée.';
            } catch(e) {
                document.getElementById('eject-icon').textContent  = '❌';
                document.getElementById('eject-title').textContent = 'Erreur réseau';
                document.getElementById('eject-msg').textContent   = 'Impossible d\'envoyer la sauvegarde.';
            }
        }
...
/**
 * Écrit un fichier dans l'IDBFS d'EmulatorJS.
 * DB confirmée : "/data/saves" (version 21), object store "FILE_DATA"
...
function writeToIndexedDB(path, bytes) {
    return new Promise((resolve) => {
        ...
        req.onerror = () => { console.warn('[GBX] IDB open error:', req.error); resolve(); };
        req.onupgradeneeded = () => {
            // DB pas encore initialisée par EJS — ne rien faire
            req.result.close();
            console.warn('[GBX] IDB: DB pas encore prête (pas de partie précédente ?)');
            resolve();
        };
        req.onsuccess = () => {
            const db         = req.result;
            const storeNames = Array.from(db.objectStoreNames);
            console.info('[GBX] IDB stores:', storeNames, '| version:', db.version);
            const storeName  = storeNames.find(s => s === 'FILE_DATA') || storeNames[0];
            if (!storeName) { db.close(); console.warn('[GBX] IDB: aucun store trouvé'); resolve(); return; }
            try {
            ...
            } catch(e) { console.warn('[GBX] IDB tx error:', e); db.close(); resolve(); }
        };
    });
}
...
function getSavePath(game) {
    ...
    // Gambatte crée toujours .srm (même pour les .gb/.gbc)
...
function readSaveFile(game) {
    try {
        ...
        // Rejeter les saves vides (moins de 64 bytes non-zéro = pas de vraie progression)
        let nonZero = 0;
        ...
        if (nonZero < 64) {
            console.info('[GBX] Save ignorée (vide) — nonZero:', nonZero);
            return null;
        }
        return data;
    } catch(e) {
...
async function uploadSaveToServer(game, saveData, silent = false) {
    ...
    return new Promise((resolve) => {
        const blob   = saveData instanceof Blob ? saveData : new Blob([saveData]);
        const reader = new FileReader();
        reader.onload = async () => {
            ...
            if (error) {
                if (!silent) showToast('❌ Erreur sauvegarde.', 'error');
                console.warn('[GBX] uploadSaveToServer error:', error);
            } else {
                if (!silent) showToast('💾 Sauvegarde envoyée !', 'success');
                console.info('[GBX] Save uploadée :', saveData.length, 'bytes');
            }
            resolve();
        };

...
async function launchGame(game) {
    ...
    if (GBX.user) {
        try {
            ...
            if (data?.save_data) {
                ...
                GBX._pendingSaveBytes = bytes;
                console.info('[GBX] Save pré-chargée :', bytes.length, 'bytes');
            }
        } catch(e) {
    ...
    // EJS_onGameStart — core prêt : on injecte la save via FS puis loadSaveFiles()
    window.EJS_onGameStart = async () => {
        ...
        if (!GBX.user || !GBX._pendingSaveBytes) return;

        const path  = getSavePath(game);
        const bytes = GBX._pendingSaveBytes;
        const fs    = GBX.emulator?.gameManager?.FS;
        if (!fs) return;

        try {
            // S'assurer que le dossier existe
            const dir = path.substring(0, path.lastIndexOf('/'));
            try { fs.mkdir(dir); } catch(_) {} // ignore si déjà existant

            // Écrire les bytes dans le FS Emscripten (mémoire WASM)
            fs.writeFile(path, bytes);
            console.info('[GBX] FS.writeFile OK:', path, bytes.length, 'bytes');

            // Demander à gambatte de relire sa SRAM depuis le FS
            GBX.emulator.gameManager.functions.loadSaveFiles();
            console.info('[GBX] loadSaveFiles() appelé');
            showToast('💾 Sauvegarde restaurée !', 'success');
        } catch(e) {
            console.warn('[GBX] Injection save échouée:', e);
        }
    };
 ...
 * EmulatorJS (Emscripten) utilise une DB nommée "/" avec l'object store "FILE_DATA".
 * La clé est le chemin absolu, la valeur est { timestamp, mode, contents }.
 */
function writeToIndexedDB(path, bytes) {
    return new Promise((resolve) => {
        // EmulatorJS nomme sa DB "/" (racine Emscripten IDBFS)
        const req = indexedDB.open('/');
        req.onerror = () => { console.warn('[GBX] IDB open error'); resolve(); };
        req.onupgradeneeded = () => {
            // DB pas encore créée — EJS ne l'a pas encore initialisée, on ne peut rien faire
            req.result.close();
            resolve();
        };
        req.onsuccess = () => {
            ...
            console.info('[GBX] IDB stores:', storeNames);
            const storeName = storeNames.find(s => s === 'FILE_DATA') || storeNames[0];
            if (!storeName) { db.close(); console.warn('[GBX] IDB: no store found'); resolve(); return; }
            try {
                const tx     = db.transaction(storeName, 'readwrite');
                const store  = tx.objectStore(storeName);
                const record = { timestamp: new Date(), mode: 33206, contents: bytes };
                const put    = store.put(record, path);
                put.onsuccess = () => { console.info('[GBX] IDB write OK:', path); db.close(); resolve(); };

...
function showEjectConfirm() {
    ...
    backdrop.innerHTML = `
        <div style="background:#1e1e2e;border-radius:16px;padding:28px 32px;max-width:320px;width:90%;text-align:center;box-shadow:0 8px 32px rgba(0,0,0,0.5);">
            ...
            <h3 style="color:#fff;margin:0 0 10px;font-size:1.1rem">Éjecter le jeu ?</h3>
            <p style="color:#aaa;font-size:0.88rem;margin:0 0 22px;line-height:1.5">Pense à sauvegarder dans le jeu avant d'éjecter,<br>puis exporte ta save depuis le menu.</p>
...
function setupSavesUI() {
    ...
    // Si un jeu est déjà en cours, pré-sélectionner
    sel.addEventListener('focus', () => {
        if (GBX.currentGame && !sel.value) {
            sel.value = GBX.currentGame.id;
        }
    });
    ...
    btn.addEventListener('click', () => {
        ...
        // Déterminer le jeu cible : en cours ou sélectionné dans la liste
        const targetGame = GBX.currentGame || GBX_GAMES.find(g => g.id === sel.value);

        if (!targetGame) {
            showToast('Sélectionne d\'abord un jeu dans la liste.', 'error');
            return;
        }
        ...
        input.addEventListener('change', async () => {
            ...
            reader.onload = async (ev) => {
                ...
                // 1. Écriture dans l'IDB (persistée pour le prochain lancement)
                await writeToIndexedDB(path, bytes);

                // 2. Si le jeu ciblé est déjà en cours, injection directe dans le FS mémoire
                ...
                showToast('✅ Save importée pour ' + targetGame.name + ' — relance le jeu pour jouer !', 'success');
            };

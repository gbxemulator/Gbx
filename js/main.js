...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    setupSaveStateUI();       // Bouton SAVE (sauvegarde rapide)
...
function setupGameSelector() {
    ...
    eject.addEventListener('click', () => {
        ...
        stopAutoSave();
        if (window._gbxBeforeUnload) {
            window.removeEventListener('beforeunload', window._gbxBeforeUnload);
            window._gbxBeforeUnload = null;
        }
...
async function launchGame(game) {
    ...
    // Force EmulatorJS à flusher la SRAM toutes les 30s nativement
    ...
    window.EJS_ready = () => {
        ...
        // Injection FS WASM (double sécurité pour GBC/GBA)
        if (GBX.user && GBX._pendingSaveArr) {
            ...
            if (gm) {
                ...
                if (path) {
                    try {
                        gm.FS.writeFile(path, GBX._pendingSaveArr);
                        gm.functions.loadSaveFiles();
                        console.info('[GBX] 📂 Save ingame restaurée (FS inject)');
                    } catch(e) {}
                }
            }
        ...
        // Flush final si l'utilisateur ferme l'onglet brutalement
        window._gbxBeforeUnload = async () => {
            const data = _getSrmData();
            if (!data || !GBX.user || !GBX.currentGame) return;
            const nonFF = Array.from(data).filter(b => b !== 0xFF).length;
            if (nonFF < data.length * 0.10) return;
            const b64 = _srmToBase64(data);
            // sendBeacon = seul moyen fiable de POST en beforeunload
            const payload = JSON.stringify({
                user_id: GBX.user.id,
                game_id: GBX.currentGame.id,
                data:    b64,
                saved_at: new Date().toISOString(),
            });
            navigator.sendBeacon('/api/save-beacon', payload); // fallback silencieux
            // Tentative async Supabase (peut ne pas aboutir si onglet fermé)
            try {
        ...
        window.addEventListener('beforeunload', window._gbxBeforeUnload);
    };
    ...
    const gbxKeys = {
        ...
        'KeyG': () => {
            ...
            if (btn && btn.style.display !== 'none') {
                addPressEffect(btn);
                openSaveModal(); // gère le cas GBA en interne
            }
        },
    };
...
function applyColor(colorId) {
    ...
    if (title) {
        if (colorId === 'bordeaux') {
            title.innerHTML = 'MEI <span style="font-size:0.7em">v</span>1';
        } else {
...
/* ============================================================
   ...
   ============================================================
   Deux types indépendants :

   • Save ingame (.srm) → table "saves"
     Sync automatique toutes les 3s si le fichier change.
     1 seul enregistrement par (user, game), écrasé à chaque fois.

   • Save rapide (state snapshot) → table "save_states"
     Slot unique (slot = 1) par (user, game).
     Déclenché manuellement via le bouton SAVE.
   ============================================================ */
...
function _getSrmData() {
    try {
        ...
        if (!gm) return null;
        // 1) Demander au core d'écrire la SRAM dans le FS WASM
        if (typeof gm.functions?.saveSaveFiles === 'function') gm.functions.saveSaveFiles();
        // 2) Lire directement depuis le FS WASM (source de vérité en RAM)
        const path = gm.functions?.getSaveFilePath?.();
        if (!path) return null;
        const data = gm.FS.readFile(path);
        if (!data || data.length === 0) return null;
        // 3) Propager WASM → IndexedDB en arrière-plan (ne pas await ici)
...
function _applySrmBase64(b64) {
    const gm = GBX.emulator?.gameManager || window.EJS_emulator?.gameManager;
    if (!gm) throw new Error('gameManager non disponible');
    const path = gm.functions?.getSaveFilePath?.();
    if (!path) throw new Error('getSaveFilePath indisponible');
    const binary = atob(b64);
    const arr = new Uint8Array(binary.length);
    for (let i = 0; i < binary.length; i++) arr[i] = binary.charCodeAt(i);
    gm.FS.writeFile(path, arr);
    if (typeof gm.functions?.loadSaveFiles === 'function') gm.functions.loadSaveFiles();
}

// ── Helpers bas niveau : save state (snapshot) ───────────────

function _captureState() {
    const gm = GBX.emulator?.gameManager || window.EJS_emulator?.gameManager;
    if (!gm) return null;
    if (!gm.functions?.supportsStates?.()) return null;
    try {
        const state = gm.Module.EmulatorJSGetState();
        if (!state || state.length === 0) return null;
        return state;
    } catch(e) { console.warn('[GBX] captureState:', e); return null; }
}

function _applyState(b64) {
    const gm = GBX.emulator?.gameManager || window.EJS_emulator?.gameManager;
    if (!gm) throw new Error('gameManager non disponible');
    const gameName = GBX.currentGame?.id || 'game';
    const statePath = '/data/saves/state_slot1_' + gameName + '.state';
    const binary = atob(b64);
    const arr = new Uint8Array(binary.length);
    for (let i = 0; i < binary.length; i++) arr[i] = binary.charCodeAt(i);
    gm.FS.writeFile(statePath, arr);
    gm.functions.loadState(statePath);
}
...
// Attend que le core ait initialisé son FS (getSaveFilePath non vide
// ET dossier du core existant) puis injecte le .srm depuis Supabase.

async function _restoreIngameSave(gameId) {
    // Fetch la save Supabase
    let saveArr = null;
    try {
        const { data: saveRow } = await _supa
            .from('saves')
            .select('data')
        ...
        if (saveRow?.data) {
            ...
            saveArr = new Int8Array(binary.length);
            for (let i = 0; i < binary.length; i++) saveArr[i] = binary.charCodeAt(i);
        }
    } catch(e) { console.warn('[GBX] fetch save:', e); }

    if (!saveArr) {
        // Pas de save — le startAutoSave sera déclenché par EJS_ready normalement
        return;
    }

    // Écrire dans IndexedDB AVANT que loader.js soit chargé
    // syncfs() copie IDB → FS WASM au boot — la save doit être là avant
    try {
        const idbPath = _getIDBSavePath(gameId);
        if (idbPath) {
            const dirPath = idbPath.substring(0, idbPath.lastIndexOf('/'));
            await _writeToIDB(dirPath, null);
            await _writeToIDB(idbPath, saveArr);
            console.info('[GBX] 📂 Save écrite dans IndexedDB', idbPath);
        }
    } catch(e) { console.warn('[GBX] IDB write:', e); }

    // Stocker pour injection FS WASM dans EJS_onGameStart
    GBX._pendingSaveArr = saveArr;
}

// Construire le chemin IndexedDB du .srm — doit correspondre EXACTEMENT
// à ce qu'EmulatorJS génère : basé sur le nom du fichier ROM (sans extension)
function _getIDBSavePath(gameId) {
    const game = GBX_GAMES.find(g => g.id === gameId);
    if (!game) return null;
    // Extraire le nom du fichier ROM depuis l'URL (sans extension)
    const romFile  = game.file.split('/').pop().split('?')[0];
    const fileName = romFile.substring(0, romFile.lastIndexOf('.'));
    const coreDir  = (game.type === 'gba') ? 'mGBA' : 'Gambatte';
    return `/data/saves/${coreDir}/${fileName}.srm`;
}

// Écrire dans IndexedDB au format IDBFS
// contents = Int8Array pour un fichier, null pour un dossier
async function _writeToIDB(path, contents) {
    const db = await new Promise((res, rej) => {
        const req = indexedDB.open('/data/saves');
        req.onsuccess = () => res(req.result);
        req.onerror = () => rej(req.error);
    });
    const mode = contents ? 33206 : 16877; // 33206 = fichier, 16877 = dossier
    const entry = contents ? { timestamp: new Date(), mode, contents } : { timestamp: new Date(), mode };
    await new Promise((res, rej) => {
        const tx = db.transaction('FILE_DATA', 'readwrite');
        const store = tx.objectStore('FILE_DATA');
        const r = store.put(entry, path);
        r.onsuccess = () => res();
...
function startAutoSave() {
    ...
    _autoSaveTimer = setInterval(async () => {
        if (!GBX.isRunning || !GBX.user || !GBX.currentGame) return;
        try {
            const data = _getSrmData();
            if (!data) return;
            // Ne pas sync si le .srm est insuffisamment initialisé
            // Seuils calibrés : GB ~2000, GBA ~40000 bytes non-FF pour une vraie save
            // Ne pas sync si moins de 10% des bytes sont non-FF (save non initialisée)
            const nonFF = Array.from(data).filter(b => b !== 0xFF).length;
            if (nonFF < data.length * 0.10) return;
            const hash = _quickHash(data);
            if (hash === _autoSaveLastHash) return;
            ...
            if (!error) console.info('[GBX] 💾 Save ingame syncée');
        } catch(e) { /* silencieux */ }
    }, 3_000);
}
...
// ── SAVE RAPIDE : sauvegarder slot 1 (table "save_states") ───

async function _quickSave(gameId) {
    const state = _captureState();
    if (!state) { showToast('⚠️ Lance un jeu d\'abord !', 'error'); return false; }
    const b64 = _srmToBase64(state);
    const { error } = await _supa
        .from('save_states')
        .upsert({
            user_id:  GBX.user.id,
    ...
    if (error) { console.error('[GBX] quickSave:', error); return false; }
    return true;
}

// ── SAVE RAPIDE : charger slot 1 (table "save_states") ───────

async function _quickLoad(gameId) {
    const { data, error } = await _supa
        ...
        .eq('slot', 1)
        .single();
    if (error || !data) { showToast('❌ Aucune save rapide pour ce jeu.', 'error'); return false; }
    try {
        _applyState(data.data);
        return true;
    } catch(e) {
        console.error('[GBX] quickLoad:', e);
        showToast('❌ Impossible de charger la save rapide.', 'error');
        return false;
    }
}

// ── Récupérer les infos du slot 1 depuis Supabase ────────────

async function _fetchQuickSlot(gameId) {
    ...
    const { data, error } = await _supa
        ...
        .eq('slot', 1)
        .maybeSingle();
    if (error) { console.warn('[GBX] fetchQuickSlot:', error); return null; }
    return data || null;
}

// ── Construire un slot visuel (load ou save) ──────────────────

function _buildQuickSlotUI(containerId, slotData, onClick, mode) {
    ...
    btn.className = 'quick-slot' + (isFilled ? ' quick-slot--filled' : '');
    if (mode === 'load' && !isFilled) btn.classList.add('quick-slot--empty-load');

    const label = document.createElement('div');
    label.className   = 'slot-label';
    label.textContent = mode === 'load' ? '📂 Charger' : '💾 Sauvegarder';

    const info = document.createElement('div');
    info.className = 'slot-info';
    if (isFilled) {
        const d = new Date(slotData.saved_at);
        info.textContent = d.toLocaleDateString('fr-FR') + ' ' + d.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
    } else {
        info.textContent = mode === 'load' ? '— aucune save —' : '— vide —';
    }

    btn.appendChild(label);
    btn.appendChild(info);
    if (!(mode === 'load' && !isFilled)) btn.addEventListener('click', onClick);
    container.appendChild(btn);
}

// ── Ouvrir la modale SAVE (split gauche=charger / droite=sauvegarder) ──
...
async function openSaveModal() {
    ...
    if (GBX.currentGame?.type === 'gba') {
        ...
        showToast('⚡ Dispo que sur GB & GBC', 'info');
        return;
    }
    const gameId = GBX.currentGame?.id;
    if (!gameId) return;

    const slotData = await _fetchQuickSlot(gameId);

    _buildQuickSlotUI('quick-load-slot', slotData, async () => {
        const ok = await _quickLoad(gameId);
        if (ok) { closeModal(); showToast('📂 Save rapide chargée !', 'success'); }
    }, 'load');

    _buildQuickSlotUI('quick-save-slot', slotData, async () => {
        const ok = await _quickSave(gameId);
        if (ok) { closeModal(); showToast('✅ Save rapide enregistrée !', 'success'); }
    }, 'save');
...
function setupSaveStateUI() {
    const btnSave = document.getElementById('btn-save');
    btnSave?.addEventListener('click', () => { addPressEffect(btnSave); openSaveModal(); });
    document.getElementById('modal-save')?.addEventListener('click', (e) => {
        if (e.target === document.getElementById('modal-save')) closeModal();
    });
}

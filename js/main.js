...
function setupPcControls() {
    ...
    volBtn?.addEventListener('click', (e) => {
        ...
        if (volOpen) { screenPanel.hidden = true; screenOpen = false; }
    });
    ...
    volPanel?.querySelectorAll('.pc-panel-dot[data-volume]').forEach(dot => {
        dot.addEventListener('click', (e) => {
            ...
            currentVol = val;
            updateVolDots(val);
            // Appliquer au media element de l'émulateur
            try {
                document.querySelectorAll('#ejs-container audio, #ejs-container video').forEach(el => {
                    el.volume = val;
                });
    ...
    function applyScreenSize(size) {
        ...
        // Quitter fullscreen si actif
        if (document.fullscreenElement) {
            document.exitFullscreen().catch(() => {});
        }

        // Réinitialiser les styles custom
        if (ejsContainer) {
            ejsContainer.style.width    = '';
            ejsContainer.style.height   = '';
            ejsContainer.style.position = '';
            ejsContainer.style.zIndex   = '';
        }

        if (size === 'default') {
            // Rien à faire — styles réinitialisés au-dessus
        } else if (size === '30' || size === '60') {
            const pct = parseInt(size) / 100;
            const vw  = window.innerWidth;
            const vh  = window.innerHeight;
            if (ejsContainer) {
                ejsContainer.style.position = 'fixed';
                ejsContainer.style.zIndex   = '500';
                ejsContainer.style.left     = '50%';
                ejsContainer.style.top      = '50%';
                ejsContainer.style.transform= 'translate(-50%, -50%)';
                ejsContainer.style.width    = Math.round(vw * pct) + 'px';
                ejsContainer.style.height   = Math.round(vh * pct) + 'px';
            }
    ...
    screenBtn?.addEventListener('click', (e) => {
        ...
        if (screenOpen) { volPanel.hidden = true; volOpen = false; }
    });
    ...
    // Quitter fullscreen → reset bouton

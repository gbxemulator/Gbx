...
function launchGame(game) {
    ...
    GBX.isRunning   = true;   // Bloquer le sélecteur immédiatement
    ...
    // Callbacks save — doivent être définis AVANT le chargement du loader
    if (GBX.user) {
    ...
    // C'est le bon moment pour injecter la save et appliquer les options
    window.EJS_onGameStart = () => {
        ...
        // Charger la save ici — le core est prêt et le FS accepte l'écriture
        if (GBX.user) {
...
function readSaveFile(game) {
    try {
        ...
        // Taille max officielle GB/GBC/GBA = 128KB (131072 octets)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btn-upload-save')?.addEventListener('click', () => {
        ...
        input.addEventListener('change', () => {
            ...
            if (file.size > 131072) {
                showToast('❌ Fichier trop lourd (max 128KB).', 'error');
                return;
            }

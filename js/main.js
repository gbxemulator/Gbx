...
function updateAuthUI() {
    ...
    if (GBX.user) {
        ...
        // Masquer le bouton Contact de la rangée PC (doublon avec btn-contact-pc)
        ...
        // Réafficher le bouton Contact dans la rangée PC
        const btnContactUnlogged = document.getElementById('btn-contact-unlogged');
...
function setupExtraButtons() {
    ...
    // Bouton Touches/Boutons → modal keymap (PC uniquement)
    ...
    // Bouton Contact non-connecté PC (dans la rangée Communauté+Contact PC)
...
function _createBubbleEl(user, index, total, color) {
    ...
    if (hasCustomAvatar) {
        ...
        img.alt = user.pseudo;
        img.onerror = () => {
            // Si l'image plante, afficher l'avatar par défaut
    ...
    // Clic → éclater
    wrap.addEventListener('click', () => _popBubble(wrap, user, color));

    canvas.appendChild(wrap);
    return wrap;
}

function _popBubble(wrap, user, color) {
    if (_poppingLocked) return;
    _poppingLocked = true;

    // Stopper l'animation flottante
    wrap.style.animationPlayState = 'paused';
    wrap.classList.add('popping');

    // Particules
    _spawnParticles(wrap, color);

    // Afficher la fiche profil après l'animation
    setTimeout(() => {
        wrap.style.display = 'none';
...
function _spawnParticles(wrap, color) {
    const canvas = document.getElementById('bubble-canvas');
    if (!canvas) return;
    ...
    // Couleurs des morceaux : couleur principale + variantes claires/sombres
    ...
    const colors   = [
        ...
        `rgb(${Math.max(0,rgb.r-50)},${Math.max(0,rgb.g-50)},${Math.max(0,rgb.b-50)})`,
        '#ffffff',
        `rgba(${rgb.r},${rgb.g},${rgb.b},0.5)`,
    ];

    // Formes : arc de cercle (morceaux de ballon) + petits éclats
    const PIECES = 14;
    for (let i = 0; i < PIECES; i++) {
        const cvs = document.createElement('canvas');
        const angle     = (Math.PI * 2 / PIECES) * i + _randomBetween(-0.2, 0.2);
        const speed     = _randomBetween(55, 110);
        const vx        = Math.cos(angle) * speed;
        const vy        = Math.sin(angle) * speed;
        const size      = _randomBetween(8, 22);
        const rot       = _randomBetween(0, Math.PI * 2);
        const rotSpeed  = _randomBetween(-6, 6);
        const pieceColor = colors[i % colors.length];

        cvs.width  = Math.ceil(size * 2);
        cvs.height = Math.ceil(size * 2);
        cvs.style.cssText = `
            position:absolute;
            left:${cx - cvs.width/2}px;
            top:${cy - cvs.height/2}px;
            pointer-events:none;
            z-index:20;
        `;

        // Dessiner un morceau de ballon (arc irrégulier)
        const ctx2d = cvs.getContext('2d');
        ctx2d.save();
        ctx2d.translate(cvs.width/2, cvs.height/2);
        ctx2d.rotate(rot);
        ctx2d.beginPath();
        // Forme organique : arc + courbe de bézier pour imiter un morceau de latex
        if (i % 3 === 0) {
            // Morceau courbé
            ctx2d.moveTo(-size * 0.5, -size * 0.2);
            ctx2d.bezierCurveTo(-size*0.8, -size*0.8, size*0.8, -size*0.8, size*0.5, -size*0.2);
            ctx2d.bezierCurveTo(size*0.3, size*0.5, -size*0.3, size*0.5, -size*0.5, -size*0.2);
        } else if (i % 3 === 1) {
            // Éclat triangulaire irrégulier
            ctx2d.moveTo(0, -size);
            ctx2d.lineTo(size * _randomBetween(0.4, 0.9), size * _randomBetween(0.3, 0.8));
            ctx2d.lineTo(-size * _randomBetween(0.4, 0.9), size * _randomBetween(0.2, 0.7));
        } else {
            // Petit bout arrondi
            ctx2d.ellipse(0, 0, size * 0.5, size * 0.9, 0, 0, Math.PI * 2);
        }
        ctx2d.closePath();
        ctx2d.fillStyle = pieceColor;
        ctx2d.fill();
        ctx2d.restore();

        canvas.appendChild(cvs);

        // Animation JS frame par frame
        const startTime = performance.now();
        const duration  = _randomBetween(420, 650);
        let   currentRot = rot;

        const animate = (now) => {
            const t    = (now - startTime) / duration;
            if (t >= 1) { cvs.remove(); return; }
            const ease = 1 - t * t; // deceleration
            const gravity = t * t * 80; // gravité
            const x = cx - cvs.width/2  + vx * t * (duration/1000);
            const y = cy - cvs.height/2 + vy * t * (duration/1000) + gravity;
            currentRot += rotSpeed * 0.016;
            cvs.style.left    = x + 'px';
            cvs.style.top     = y + 'px';
            cvs.style.opacity = ease;
            cvs.style.transform = `rotate(${currentRot}rad)`;
...
function _showBubbleProfile(user, color, bubbleWrap) {
    ...
    lastGameEl.textContent = user.last_game ? '🎮 ' + user.last_game : '';
...
async function _loadCommunauteUsers() {
    try {
        ...
        users.forEach((user, i) => {
            const color = GBX_BUBBLE_COLORS[i % GBX_BUBBLE_COLORS.length];

...
async function launchGame(game) {
    ...
    window.EJS_ready = () => {
        ...
        // L'auto-save démarre après la restauration via _restoreIngameSave
        // qui est lancée avant loader.js
    };

    window.EJS_onGameStart = () => {
        // Injection FS WASM pour GBC/GBA (loadSaveFiles fonctionne en cours de partie)
        if (GBX.user) {
            const gm = window.EJS_emulator?.gameManager;
            if (!gm) return;
            const path = gm.functions?.getSaveFilePath?.();
            if (!path || !GBX._pendingSaveArr) return;
            try {
    ...
    if (GBX.user) {
        _restoreIngameSave(game.id).then(() => {
            // Délai 300ms pour laisser la transaction IDB se commiter
            // avant que syncfs() copie IDB → FS WASM au boot
            setTimeout(() => {

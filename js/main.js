...
function _spawnParticles(wrap, color) {
    // On crée un canvas unique en position fixed qui couvre tout l'écran
    // Aucun problème de coordonnées relatives
    ...
    // Centre de la bulle en coordonnées écran (fixed)
    ...
    const lighter   = `rgb(${Math.min(255,rgb.r+70)},${Math.min(255,rgb.g+70)},${Math.min(255,rgb.b+70)})`;
    const darker    = `rgb(${Math.max(0,rgb.r-60)},${Math.max(0,rgb.g-60)},${Math.max(0,rgb.b-60)})`;
    const pieceColors = [safeColor, lighter, darker, '#ffffff', safeColor, lighter];
    ...
    for (let i = 0; i < PIECES; i++) {
        ...
        pieces.push({
            ...
            shape:    i % 3,   // 0=arc, 1=triangle, 2=ellipse
    ...
    const draw = (now) => {
        ...
        pieces.forEach(p => {
            ...
            if (p.shape === 0) {
                // Arc courbé façon morceau de ballon
                ctx.moveTo(-p.w * 0.5, -p.h * 0.2);
                ctx.bezierCurveTo(-p.w, -p.h, p.w, -p.h, p.w * 0.5, -p.h * 0.2);
                ctx.bezierCurveTo(p.w * 0.3, p.h * 0.6, -p.w * 0.3, p.h * 0.6, -p.w * 0.5, -p.h * 0.2);
            } else if (p.shape === 1) {
                // Triangle irrégulier
                ctx.moveTo(0, -p.h);
                ctx.lineTo(p.w * 0.7, p.h * 0.6);
                ctx.lineTo(-p.w * 0.6, p.h * 0.5);
            } else {
                // Ellipse
                ctx.ellipse(0, 0, p.w * 0.5, p.h * 0.8, 0, 0, Math.PI * 2);
            }

    ...
    const poll = () => {
        ...
        for (const pad of pads) {
            ...
            pad.buttons.forEach((btn, i) => {
                ...
                // Chercher quelle action correspond à ce bouton
                for (const [actionId, btnIndex] of Object.entries(map)) {
                    ...
                    if (el && !el.disabled) el.classList.add('pressed');
                    const gm = GBX.emulator?.gameManager;
                    if (gm) try { gm.simulateInput(0, action.ejsIndex, 1); } catch(_) {}
                }
            });
            // Axes → D-PAD
            const axH = pad.axes[0]; // horizontal
            const axV = pad.axes[1]; // vertical
            const gm = GBX.emulator?.gameManager;
            if (gm) {
                try {
                    gm.simulateInput(0, 6, axH < -0.5 ? 1 : 0); // gauche
                    gm.simulateInput(0, 7, axH >  0.5 ? 1 : 0); // droite
                    gm.simulateInput(0, 4, axV < -0.5 ? 1 : 0); // haut
                    gm.simulateInput(0, 5, axV >  0.5 ? 1 : 0); // bas
                } catch(_) {}
            }
        }

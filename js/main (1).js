...
async function _restoreIngameSave(gameId) {
    ...
    try {
        ...
        if (saveRow?.data) {
            ...
            saveArr = new Uint8Array(binary.length);
    ...
    // Injecter dès que le FS WASM est dispo — on crée le dossier nous-mêmes
    // pour être le plus tôt possible (avant que le core lise la mémoire au boot)
    let attempts = 0;
    const MAX = 60; // 60 × 100ms = 6s max
    ...
    const tryInject = () => {
        ...
        try {
            ...
            try { gm.FS.mkdir('/data'); } catch(e) {}
            try { gm.FS.mkdir('/data/saves'); } catch(e) {}

            // Déterminer le dossier du core
            const path = gm.functions?.getSaveFilePath?.() || '';
            ...
            if (dir) {
                ...
                try {
                    ...
                    console.info('[GBX] 📂 Save ingame restaurée (early inject)');
                    // Aussi appeler loadSaveFiles pour GBC/GBA qui peuvent recharger en cours de partie

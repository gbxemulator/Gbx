...
function _restoreIngameSave(gameId) {
    let attempts = 0;
    const MAX = 40; // 40 × 300ms = 12s max

    const tryInject = async () => {
        attempts++;
        if (attempts > MAX) { startAutoSave(); return; }

        try {
            const gm = window.EJS_emulator?.gameManager;
            if (!gm?.functions?.getSaveFilePath) { setTimeout(tryInject, 300); return; }

            const path = gm.functions.getSaveFilePath();
            if (!path) { setTimeout(tryInject, 300); return; }

            const dir = path.substring(0, path.lastIndexOf('/'));
            try { gm.FS.readdir(dir); } catch(e) { setTimeout(tryInject, 300); return; }

            // FS prêt — fetch la save
            ...
            if (saveRow?.data) {
                ...
                console.info('[GBX] 📂 Save ingame restaurée');
            }
        } catch(e) { console.warn('[GBX] _restoreIngameSave:', e); }

        // Démarrer l'auto-save après la restauration (ou si pas de save)

...
function _captureState() {
    ...
    try {
        // Méthode 1 : gambatte (GB/GBC) — API bas niveau
        ...
        // Méthode 2 : mgba (GBA) — API fichier EJS
        // saveState() écrit dans le FS WASM, on lit ensuite le fichier
        const gameName = GBX.currentGame?.id || 'game';
        const statePath = '/data/saves/' + gameName + '.state1';
        try { gm.FS.unlink(statePath); } catch(_) {}
        if (typeof gm.functions?.saveState === 'function') {
            gm.functions.saveState(statePath);
        } else {
            return null;
        }
        // Lire le fichier écrit par saveState
        const data = gm.FS.readFile(statePath);
        if (!data || data.length === 0) return null;
        return data;
...
function _applyState(b64) {
    ...
    if (typeof gm.functions?.loadState === 'function') {
        ...
        throw new Error('loadState indisponible');
    }
}
